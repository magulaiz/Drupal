...
final class FunctionalTestDoesNotAccessContainerGet implements Rule {
  ...
  public function processNode(Node $node, Scope $scope): array {
    ...
    if ($node instanceof MethodCall && $node->name instanceof Identifier && $node->name->name === 'get') {
      $var = $node->var;
      if ($var instanceof PropertyFetch && $var->name instanceof Identifier && $var->name->name === 'container') {
        return [
