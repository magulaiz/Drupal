...
class SplitbuttonTest extends WebDriverTestBase {
  ...
  public function testSplitbuttons($theme_name) {
    ...
    $scenarios = [
      'splitbutton_link_first' => [
        'primary_selector' => '[data-drupal-selector="splitbutton-main"] > a:first-child',
        'number_links' => 3,
        'number_submit' => 2,
        'number_items' => 5,
      ],
      'splitbutton_submit_first' => [
        'primary_selector' => '[data-drupal-selector="splitbutton-main"] > input:first-child',
        'number_links' => 4,
        'number_submit' => 2,
        'number_items' => 6,
      ],
      'splitbutton_with_title' => [
        'primary_selector' => '[data-drupal-selector="splitbutton-trigger"]',
        'number_links' => 4,
        'number_submit' => 2,
        'number_items' => 6,
      ],
    ...
    foreach ($button_types as $button_type) {
      foreach ($scenarios as $scenario => $scenario_data) {
        ...
        $toggle = $splitbutton->find('css', '[data-drupal-selector="splitbutton-trigger"]');
        ...
        if (!empty($theme_name)) {
          ...
          if ($button_type !== 'default') {
            $assert_session->elementExists('css', "$splitbutton_selector.splitbutton.splitbutton--$button_type");
          }
        }
      }
    }
    ...
    foreach ($test_ids as $test_id) {
      ...
      $this->assertFalse($splitbutton->hasAttribute('data-drupal-splitbutton-initialized'));
      $this->assertTrue($splitbutton->hasAttribute('data-drupal-splitbutton-single'));
      $this->assertNull($splitbutton->find('css', '[data-drupal-selector="splitbutton-item-list"]'));
    }
  }
  ...
  public function testSplitbuttonKeyboard() {
    ...
    $script = <<<EndOfScript
(document.activeElement.tagName === 'INPUT') ? document.activeElement.getAttribute("value") : document.activeElement.textContent
EndOfScript;
    ...
    foreach ($key_steps as $iteration => $step) {
      ...
      $focused_element_text = $this->getSession()->evaluateScript($script);
      $this->assertEquals($step['expected_destination'], array_search($focused_element_text, $menu_item_values), "Unexpected focus on step $iteration.");
    }
    ...
    $this->assertJsCondition('document.querySelector("[data-drupal-selector=\'splitbutton-trigger\']") === document.activeElement');
  ...
  /**
   * Confirm splitbutton can use other element types.
   */
  public function testSplitbuttonWithAdditionalElements() {
    $assert_session = $this->assertSession();

    $toggle = $assert_session->waitForElementVisible('css', '[data-splitbutton-test-id="splitbutton-other-elements"] [data-drupal-selector="splitbutton-trigger"]');
    $this->assertNotNull($toggle);
    $toggle->press();

    $h4 = $assert_session->waitForElementVisible('css', '[data-splitbutton-test-id="splitbutton-other-elements"] [data-drupal-selector="splitbutton-item-list"] h4');
    $this->assertNotNull($h4);
    $this->assertSame('I am an H4', $h4->getText());
    $number = $assert_session->waitForElementVisible('css', '[data-splitbutton-test-id="splitbutton-other-elements"] [data-drupal-selector="splitbutton-item-list"] [type="number"]');
    $this->assertNotNull($number);
    $this->assertSame('89', $number->getValue());
    $checkbox = $assert_session->waitForElementVisible('css', '[data-splitbutton-test-id="splitbutton-other-elements"] [data-drupal-selector="splitbutton-item-list"] [type="number"]');
