...
use Drupal\Core\TypedData\TypedDataResolverInterface;
...
class TypedDataResolverTest extends KernelTestBase {
   ...
   */
  protected TypedDataResolverInterface $typedDataResolver;
  ...
  public function testGetContextFromProperty(): void {
    ...
    $this->assertSame('Test name', $property_context->getContextValue());
    ...
    $this->assertSame('mail@example.org', $property_context->getContextValue());
  }

  /**
   * @covers ::convertTokenToContext
   */
  public function testConvertTokenToContext(): void {
    ...
    $this->assertSame('language', $context->getContextDefinition()->getDataType());

    // Test the reference to the name.
    $context = $this->typedDataResolver->convertTokenToContext('entity_test:user_id:entity', $contexts);
    $this->assertSame('entity:user', $context->getContextDefinition()->getDataType());

    $context = $this->typedDataResolver->convertTokenToContext('entity_test:user_id:entity:mail:value', $contexts);
    $this->assertSame('email', $context->getContextDefinition()->getDataType());
  }

  /**
   * @covers ::getLabelByToken
   */
  public function testGetLabelByToken(): void {
    $context_definition = new EntityContextDefinition($this->entity->getTypedData()->getDataDefinition()->getDataType(), 'Context definition label');
    $contexts['entity_test'] = new Context($context_definition, $this->entity);

    // Test the language property.
    $this->assertSame('Context definition label: Language', $this->typedDataResolver->getLabelByToken('entity_test:langcode:language', $contexts));

    // Test the reference to the user.
    $this->assertSame('Context definition label: User ID', $this->typedDataResolver->getLabelByToken('entity_test:user_id:entity', $contexts));
  }

  /**
   * @covers ::getTokensForContexts
   */
  public function testGetTokensForContexts(): void {
    $context_definition = new EntityContextDefinition($this->entity->getTypedData()->getDataDefinition()->getDataType(), 'Context definition label');
    $contexts['entity_test'] = new Context($context_definition, $this->entity);
    $this->assertEquals([
   ...
   */
  protected function assertPropertyPath(ContentEntityInterface $entity, $property_path, $expected_data_type): ContextInterface {
