...
class EntityReferenceFieldTest extends EntityKernelTestBase {
  ...
  public function testEntityReferenceFieldValidation() {
    ...
    $this->assertEquals('The referenced entity ('.$this->referencedEntityType.': 9999) does not exist.', $violations[0]->getMessage());
    ...
    $violations = $entity->{$this->fieldName}->validate();
    $this->assertEquals(1, $violations->count(), 'Validation throws a violation.');
    $this->assertEquals('This entity '.$this->referencedEntityType.': '.$referenced_entity->id().' cannot be referenced.', $violations[0]->getMessage());
  }
