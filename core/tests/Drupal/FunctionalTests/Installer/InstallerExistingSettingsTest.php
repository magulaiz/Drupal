...
class InstallerExistingSettingsTest extends InstallerTestBase {
  ...
  protected function visitInstaller(): void {
    ...
    // Ensure only the database_file_cache database table has been created. This
    // is because it can be created via a container rebuild without specific
    // interaction with the installer.
    $this->assertSame(['database_file_cache' => 'database_file_cache'], Database::getConnection()->schema()->findTables('%'));
