...
class ClassyPreprocessUnchangedTest extends UnitTestCase {
  ...
  public function testNoNewPreprocess() {
    ...
    $hash = hash('xxh64', $classy_theme_contents);
    $this->assertSame($hash, 'abcca497524df976', "The file hash for classy.theme has changed. Any additions or changes to preprocess functions should be added to the themes that inherit Classy. \nIf the changes to classy.theme are not changes to preprocess functions, update the hash in this test to: '$hash' so it will pass.");
  }
