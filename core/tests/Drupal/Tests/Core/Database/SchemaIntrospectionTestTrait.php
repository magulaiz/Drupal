...
trait SchemaIntrospectionTestTrait {
   ...
   */
  protected function assertIndexOnColumns(string $table_name, array $column_names, string $index_type = 'index') {
   ...
   *   'primary'. Defaults to 'index'.
   */
  protected function assertNoIndexOnColumns(string $table_name, array $column_names, string $index_type = 'index') {
    foreach ($this->getIndexColumnNames($table_name, $index_type) as $index_columns) {
      if ($column_names == $index_columns) {
        $this->assertTrue(FALSE);
      }
   ...
   *   the given type.
   */
  protected function getIndexColumnNames(string $table_name, string $index_type) {
