...
class DatabaseBackend implements CacheBackendInterface {
  ...
  public function getMultiple(&$cids, $allow_invalid = FALSE) {
    ...
    $result = [];
    try {
      $result = $this->connection->query('SELECT [cid], [data], [created], [expire], [serialized], [tags], [checksum] FROM {' . $this->bin . '} WHERE [cid] IN ( :cids[] ) ORDER BY [cid]', [':cids[]' => array_keys($cid_mapping)])->fetchAll();
    }
    catch (\Exception) {
      // Nothing to do.
    }
