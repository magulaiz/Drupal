      ...
      if ($entity_type_id = $this->configManager->getEntityTypeIdByName($config_name)) {
        ...
        if ($bundle_of = $entity_type->getBundleOf()) {
          ...
          if (!empty($entity_ids)) {
            ...
            $event->getConfigImporter()->logError($this->t('Entities exist of type %entity_type and %bundle_label %bundle (%bundle_id). These entities need to be deleted before importing.', ['%entity_type' => $bundle_of_entity_type->getLabel(), '%bundle_label' => $bundle_of_entity_type->getBundleLabel(), '%bundle' => $entity->label(), '%bundle_id' => $bundle_id]));
          }
        }
      }
    }
