    ...
    if (empty($this->bundleInfo)) {
      ...
      if ($cache = $this->cacheGet("entity_bundle_info:$langcode")) {
        ...
        foreach ($this->entityTypeManager->getDefinitions() as $type => $entity_type) {
          // First look for entity types that act as bundles for others,
          // load them and add them as bundles.
        ...
        $this->cacheSet("entity_bundle_info:$langcode", $this->bundleInfo,
        Cache::PERMANENT, ['entity_types', 'entity_bundles']);
      }
    }
