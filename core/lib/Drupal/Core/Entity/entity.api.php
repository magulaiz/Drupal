 ...
 * - The 'id' attribute argument gives the entity type ID, and the 'label'
 *   property gives the human-readable name of the entity type. If you are
 *   defining a content entity type that uses bundles, the 'bundle_label'
 ...
 * - The attribute will refer to several handler classes, which you will also
 *   need to define:
 ...
 *     attribute argument has value TRUE), define a class that extends
 ...
 *     entities can just use the 'admin_permission' attribute argument instead.
 *     Note that if you are creating your own access control handler, you should
 *     override the checkAccess() and checkCreateAccess() methods, not access().
 ...
 * - For content entities, the attribute will refer to a number of database
 *   tables and their fields. These attribute arguments, such as 'base_table',
 *   'data_table', 'entity_keys', etc., are documented on
 *   \Drupal\Core\Entity\EntityType.
 * - For content entities that are displayed on their own pages, the attribute
 ...
 *   These go into the 'links' attribute argument, with the link type as the
 *   key, and the path of this link template as the value. The corresponding
 *   route requires the following route name:
 ...
 *   attribute argument, giving the name of the route that the Manage Fields,
 ...
 *   attribute argument) of this configuration entity class goes into the
 *   'bundle_entity_type' attribute argument on the entity type class. For
 *   example, for the Node entity, the bundle class is
 *   \Drupal\node\Entity\NodeType, whose machine name is 'node_type'. This is
 *   the attribute argument 'bundle_entity_type' on the \Drupal\node\Entity\Node
 *   class. Also, the bundle config entity type attribute argument must have a
 *   'bundle_of' property, giving the machine name of the entity type it is
 *   acting as a bundle for. These machine names are considered permanent, they
 *   may not be renamed.
 * - Additional attribute arguments can be seen on entity class examples such
 *   as \Drupal\node\Entity\Node (content) and \Drupal\user\Entity\Role
 *   (configuration). These attribute arguments are documented on
 *   \Drupal\Core\Entity\EntityType.
 *
 ...
 * reference it in the attribute on the entity class: see the end of this
 ...
 *   and a form handler type from the entity attributes (see @ref define above
 *   for more on handlers and attributes). So, in this example, block.default
 ...
 * attribute argument on the entity class), and note that you should use
 ...
 * Entity types define their access permission scheme in their attribute.
 ...
 * \Drupal\Core\Entity\Attribute\EntityType and all the ones additionally
