...
class StringPartsConstraintValidator extends ConstraintValidator {
  ...
  public function validate(mixed $value, Constraint $constraint) {
    ...
    // If it's not a Mapping, it's a logical error in the config schema, not the
    // concrete config.
    ...
    $missing_elements = array_diff($constraint->parts, array_keys($elements));
    if (!empty($missing_elements)) {
      throw new \LogicException(sprintf('This validation constraint is configured to inspect the mapping elements %s, but some do not exist: %s.',
    ...
    foreach ($constraint->parts as $part) {
      $part_value = $elements[$part]->getValue();
