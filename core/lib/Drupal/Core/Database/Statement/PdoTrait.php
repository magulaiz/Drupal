...
trait PdoTrait {
   ...
   *   Not implemented in all database drivers, don't use.
   *
   * @return array<scalar>|object|scalar|false
   *   A result, formatted according to $mode, or FALSE on failure.
   */
  protected function clientFetch(?FetchAs $mode = NULL, ?int $cursorOrientation = NULL, ?int $cursorOffset = NULL): array|object|int|float|string|bool {
   ...
   *   no more rows.
   */
  protected function clientFetchColumn(int $column = 0): int|float|string|bool {
   ...
   *   constructor.
   *
   * @return array<array<scalar>|object|scalar>
   *   An array of results.
   */
