...
trait MenuLinkTrait {
  ...
  public function updateParentLinksNonJs(array $form, FormStateInterface $form_state) {
    $selected_menu = $form_state->getValue('menu_parent');

    $menu_of_selected_type = $this->getParentLinkSelectList($form_state->getValue('menu_parent_child'), $selected_menu);
  ...
  public function updateParentLinks(array $form, FormStateInterface $form_state) {
    ...
    $form['menu_parent_child']['#options'] = $menu_of_selected_type;
    return $form['menu_parent_child'];
  }
  ...
  protected function buildMenuFormElements(array $form, FormStateInterface $form_state, array $all_menu_links, string $menu_parent) {
    ...
    $form['menu_parent']['#options'] = $parent_menu_links;
    $form['menu_parent']['#title'] = $this->t('Menu');
    $form['menu_parent']['#description'] = $this->t('Select menu');
    ...
    $form['menu_parent_child']['#weight'] = 10;
    $form['menu_parent_child']['#options'] = $menu_of_selected_type;
    $form['menu_parent_child']['#title'] = $this->t('Parent link');
    $form['menu_parent_child']['#value'] = $this->allMenuLinks;
    $form['menu_parent_child']['#description'] = $this->t('The maximum depth for a link and all its children is fixed. Some menu links may not be available as parents if selecting them would exceed this limit.');
    $form['menu_parent_child']['#attributes']['class'][] = 'menu-title-select';
    $form['menu_parent_child']['#prefix'] = '<div id="ajax-updated-section">';
    $form['menu_parent_child']['#suffix'] = '</div>';
