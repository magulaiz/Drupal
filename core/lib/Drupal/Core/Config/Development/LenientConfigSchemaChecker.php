...
class LenientConfigSchemaChecker extends ConfigSchemaChecker {
  ...
  public function onConfigSave(ConfigCrudEvent $event): void {
    try {
      ...
      $message = sprintf('%s. These errors mean there is configuration that does not comply with it\'s schema. This is not a fatal error, but it is recommended to fix these issues. For more information on configuration schema\'s check out <a href="%s">the documentation</a>.', $exception->getMessage(), 'https://www.drupal.org/docs/drupal-apis/configuration-api/configuration-schemametadata');
