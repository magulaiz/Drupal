...
class Renderer implements RendererInterface {
  ...
  protected function doRender(&$elements, $is_root_call = FALSE) {
    ...
    if (isset($elements['#lazy_builder'])) {
      ...
      $callbackArgs = $elements['#lazy_builder'][1];
      assert(array_walk($callbackArgs, function ($arg, $key) {
        assert(is_null($arg) || is_scalar($arg),
          "A #lazy_builder callback's context may only contain scalar values or NULL. Problem with parameter: " . $key);
      }), "Assertion has failed for one or more parameters in callback context.");
