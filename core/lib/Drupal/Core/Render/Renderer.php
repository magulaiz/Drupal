...
class Renderer implements RendererInterface {
  ...
  protected function doRender(&$elements, $is_root_call = FALSE) {
    ...
    if (is_array($elements)) {
      $elements = \Drupal::service('plugin.manager.element_info')->createInstance('generic')->fromArray($elements);
    }
    ...
    if (isset($elements['#create_placeholder']) && $elements['#create_placeholder'] === TRUE && $this->requestStack->getCurrentRequest()->isMethodCacheable()) {
      ...
      $elements = \Drupal::service('plugin.manager.element_info')->createInstance('generic')->fromArray($placeholder);
    }
    ...
    if (isset($elements['#theme_wrappers']) && !isset($elements['#render_children'])) {
      foreach ($elements['#theme_wrappers'] as $key => $value) {
        ...
        $elements['#children'] = $this->theme->render($wrapper_hook, $wrapper_elements->toArray());
      }
    }
   ...
   * {@inheritdoc}
   */
  public function addCacheableDependency(array|RenderableElementInterface &$elements, $dependency) {
