...
class Renderer implements RendererInterface {
  ...
  protected function doRender(&$elements, $is_root_call = FALSE) {
    ...
    if (isset($elements['#access'])) {
      ...
      if ($elements['#access'] instanceof AccessResultInterface) {
        $this->addCacheableDependency($elements->asArray(), $elements['#access']);
    ...
    if (isset($elements['#cache']['keys'])) {
      ...
      if ($cached_element !== FALSE) {
        ...
        // for the subtree.
        $context->update($elements->asArray());
    ...
    $pre_bubbling_elements = array_intersect_key($elements->toArray(), [
      '#cache' => TRUE,
      '#lazy_builder' => TRUE,
    ...
    if (isset($elements['#lazy_builder'])) {
      ...
      assert(!Element::children($elements), sprintf('When a #lazy_builder callback is specified, no children can exist; all children must be generated by the #lazy_builder callback. You specified the following children: %s.', implode(', ', Element::children($elements->asArray()))));
      $supported_keys = [
        '#lazy_builder',
        '#cache',
        '#create_placeholder',
      ...
      assert(empty(array_diff(array_keys($elements->toArray()), $supported_keys)), sprintf('When a #lazy_builder callback is specified, no properties can exist; all properties must be generated by the #lazy_builder callback. You specified the following properties: %s.', implode(', ', array_diff(array_keys($elements->toArray()), $supported_keys))));
    }
    // Determine whether to do auto-placeholdering.
    if ($this->placeholderGenerator->canCreatePlaceholder($elements->toArray()) && $this->placeholderGenerator->shouldAutomaticallyPlaceholder($elements->toArray())) {
    ...
    if (isset($elements['#lazy_builder'])) {
      ...
      CacheableMetadata::createFromRenderArray($elements->toArray())
    ...
    if (!empty($elements['#states'])) {
      FormHelper::processStates($elements->asArray());
    }

    // Get the children of the element, sorted by weight.
    $children = Element::children($elements->asArray(), TRUE);
    ...
    if ($theme_is_implemented && !isset($elements['#render_children'])) {
      $elements['#children'] = $this->theme->render($elements['#theme'], $elements->toArray());
    ...
    if (isset($elements['#theme_wrappers']) && !isset($elements['#render_children'])) {
      foreach ($elements['#theme_wrappers'] as $key => $value) {
        ...
        $wrapper_elements = $elements->toArray();
        if (is_string($key)) {
          $wrapper_hook = $key;
    ...
    if (isset($pre_bubbling_elements['#cache']['keys']) && isset($elements['#cache']['keys'])) {
      ...
      $this->renderCache->set($elements->asArray(), $pre_bubbling_elements);
   ...
   *
   * @param \Drupal\Core\Render\RenderableElementInterface $elements
   ...
   *
   * @return \Drupal\Core\Render\RenderableElementInterface
   ...
   */
  protected function ensureMarkupIsSafe(RenderableElementInterface $elements) {
   ...
   */
  protected function addDebugOutput(RenderableElementInterface $elements, bool $is_cache_hit, array $pre_bubbling_elements = [], float $render_time = 0) {
