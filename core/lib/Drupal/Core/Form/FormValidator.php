...
class FormValidator implements FormValidatorInterface {
  ...
  protected function performRequiredValidation(&$elements, FormStateInterface &$form_state) {
    ...
    $value = is_array($elements['#value']) ? reset($elements['#value']) : $elements['#value'];
    if (isset($elements['#maxlength']) && mb_strlen($value) > $elements['#maxlength']) {
      $form_state->setError($elements, $this->t('@name cannot be longer than %max characters but is currently %length characters long.', ['@name' => empty($elements['#title']) ? $elements['#parents'][0] : $elements['#title'], '%max' => $elements['#maxlength'], '%length' => mb_strlen($value)]));
    }
