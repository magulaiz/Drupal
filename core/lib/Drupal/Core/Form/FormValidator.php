...
class FormValidator implements FormValidatorInterface {
  ...
  protected function performRequiredValidation(&$elements, FormStateInterface &$form_state) {
    ...
    if (isset($elements['#maxlength'])) {
      if (!is_array($elements['#value']) && mb_strlen($elements['#value']) > $elements['#maxlength']) {
        ...
        foreach ($elements['#value'] as $key => $value) {
          if (mb_strlen($value) > $elements['#maxlength']) {
            $name = empty($elements['#title']) ? $elements['#parents'][0] : $elements['#title'];
            if (isset($elements[$key]) && isset($elements[$key]['#title'])) {
              $name = $elements[$key]['#title'];
            }
            $form_state->setError($elements, $this->t('@name cannot be longer than %max characters but is currently %length characters long.', ['@name' => $name, '%max' => $elements['#maxlength'], '%length' => mb_strlen($value)]));
          }
        }
      }
    }
