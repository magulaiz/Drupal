...
class Xss {
   ...
   * @param string[]|null $allowable_html_tags
   ...
   */
  public static function filter($string, array $allowable_html_tags = NULL) {
    if (is_null($allowable_html_tags)) {
      $allowable_html_tags = static::$htmlTags;
    }
    ...
    $allowable_html_tags = array_flip($allowable_html_tags);
    // Late static binding does not work inside anonymous functions.
    $class = static::class;
    $splitter = function ($matches) use ($allowable_html_tags, $class) {
      return $class::split($matches[1], $allowable_html_tags, $class);
    };
