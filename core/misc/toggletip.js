...
((
  ...
  Drupal.behaviors.toggleTip = {
    attach(context) {
      ...
      /**
       * Closes every open toggletip on the page.
       *
       * @param {string} exceptId
       *   Optional. Will not close a toggletip with an id that matches this.
       */
      const closeAllToggletips = (exceptId) => {
        document
          .querySelectorAll(
            '[data-drupal-toggletip-toggle-button][aria-expanded="true"]',
          )
          .forEach((toggleButton) => {
            if (toggleButton.id === exceptId) {
              return;
            }
      ...
      once('drupal-tip', '[data-drupal-toggletip]', context).forEach(
        (tipElement) => {
          ...
          // Disclosure button click behavior.
          button.addEventListener('click', () => {
            const expanded = button.getAttribute('aria-expanded') === 'true';
            button.setAttribute('aria-expanded', !expanded);
            tip.hidden = !!expanded;

            // Close all toggletips other than the one just opened.
            if (expanded) {
              closeAllToggletips(button.id);
            }
          });

          // When the disclosure button is blurred, close all toggletips unless
          // focus has moved to the tip the disclosure button opens.
          button.addEventListener('blur', () => {
            // Wrap in a timeout so there is sufficient time for focus to have
            // moved.
            setTimeout(() => {
              if (document.activeElement.id !== tip.id) {
                closeAllToggletips();
              }
            });
          });

          // When the toggletip is blurred, close all toggletips unless focus has
          // moved to that tip's disclosure button.
          tip.addEventListener('blur', () => {
            // Wrap in a timeout so there is sufficient time for focus to have
            // moved.
            setTimeout(() => {
              if (document.activeElement.id !== button.id) {
                closeAllToggletips();
              }
            });
          });
          ...
          autoUpdate(button, tip, () => {
            ...
            computePosition(button, tip, {
              ...
              const { marginLeft, marginRight } = window.getComputedStyle(tip);
              const marginOffset =
      ...
      once('toggletip-escape', document.body).forEach(() => {
        // All toggletips close if escape is pressed.
