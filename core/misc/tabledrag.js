...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.TableDrag = class {
    constructor(table, tableSettings) {
       ...
       */
      this.changedRowIds = Drupal.TableDrag.instances[table.id]
        ? Drupal.TableDrag.instances[table.id].changedRowIds
        : new Set();
      ...
      if (this.indentEnabled) {
        ...
        const countNestableRows = Array.from(
          this.table.querySelectorAll(
            ':scope > tr.draggable, :scope > tbody > tr.draggable',
          ),
        ).filter((row) => {
          return !row.classList.contains('.tabledrag-leaf');
        }).length;
     ...
     *
     * In order to adjust colspan instead of hiding them altogether.
     *
    ...
    makeDraggable(row) {
      ...
      const handle = this.constructor.stringToElement(
        Drupal.theme('tableDragHandle', this.dragOrientation),
      );
      if (this.dragOrientation === 'drag-y') {
        handle.classList.add('tabledrag-handle-y');
      }

      const indentations = row.querySelectorAll(
        'td:first-of-type .js-indentation',
      );
      const indentationLast = indentations.length
      ...
      // update values after a row has ben moved through the keyboard support.
        ...
        switch (event.keyCode) {
          ...
          case 63233: {
            ...
            while (nextRow && Drupal.elementIsHidden(nextRow)) {
              nextRow = this.constructor.next(nextRow, 'tr');
            }
            if (nextRow) {
              // Do not allow the onBlur cleanup.
              this.safeBlur = false;
              ...
              if (row.matches('.tabledrag-root')) {
                ...
                if (nextGroup) {
                  nextGroup.group.forEach((groupRow) => {
                    groupHeight += Drupal.elementIsHidden(groupRow)
                      ? 0
                      : this.offsetHeight;
                  });
    ...
    dragStart(event, row) {
      ...
      if (this.rowObject) {
        const handle =
          this.rowObject.element.querySelector('a.tabledrag-handle');
        if (handle) {
          handle.blur();
        }
      }
    ...
    dragRow(event) {
      if (this.dragObject) {
        ...
        if (this.indentEnabled) {
          ...
          // Indent the row with our estimated diff, witch may be further
    ...
    dropRow(event) {
      ...
      if (this.dragObject !== null) {
        this.dragObject = null;
        document.body.classList.remove(this.dragOrientation);
        clearInterval(this.scrollInterval);
      }
    }
    ...
    /**
     * Get the event offset from target row element.
     *
    ...
    getPointerOffset(row, event) {
      ...
      return {
        x: pointerPos.x - (rowRect.x + row.ownerDocument.defaultView.scrollX),
        y: pointerPos.y - (rowRect.y + row.ownerDocument.defaultView.scrollY),
      };
    }
    ...
    findDropTargetRow(x, y) {
      const rows = [].slice
        ...
        .filter((row) => Drupal.elementIsVisible(row));
      for (let n = 0; n < rows.length; n++) {
        let row = rows[n];
        const rowY =
          row.getBoundingClientRect().top +
          row.ownerDocument.defaultView.pageYOffset;
        const rowHeight = parseInt(row.offsetHeight, 10) / 2;
        ...
        if (y > rowY - rowHeight && y < rowY + rowHeight) {
          ...
          while (
            ...
            Drupal.elementIsHidden(this.constructor.previous(row, 'tr'))
          ) {
     ...
     *  DOM object for the row that was just dropped.
     * @param {string} group
     *  The settings group on which field update will occur.
     */
    updateField(changedRow, group) {
      ...
      // Parents, look upp the tree until we find a field not in this group.
        ...
        switch (rowSettings.action) {
          ...
          case 'order': {
            ...
            if (targetElement.matches('select')) {
              ...
              if (siblings.length) {
                weight =
                  parseInt(siblings[0].querySelector(targetClass).value, 10) ||
                  0;
              }
              siblings.forEach((sibling) => {
    ...
    /**
     * Command to restripe table property.
     */
    restripeTable() {
      ...
      draggableRows.forEach((row) => {
        ...
        while (previousSibling) {
          if (Drupal.elementIsVisible(previousSibling)) {
            i += 1;
          }
    ...
    addChangedWarning() {
      ...
      if (
        !this.table.parentNode.querySelectorAll('.tabledrag-changed-warning')
          .length
      ) {
     ...
     *  Do\OM object for the row after the tested position
     ...
     */
    indent(indentDiff) {
      // Determine the valid indentations interval if not available yes.
      if (!this.interval) {
      ...
      for (let d = 0; d < directions.length; d++) {
        ...
        while (checkRow) {
          ...
          if (checkRow.querySelector(`.${rowSettings.target}`)) {
            ...
            if (this.indentEnabled) {
              checkRowIndentation =
                checkRow.querySelectorAll('.js-indentation').length;
            }
   ...
   */
  Drupal.theme.tableDragHandle = (dragOrientation = 'drag') => {
    const title =
