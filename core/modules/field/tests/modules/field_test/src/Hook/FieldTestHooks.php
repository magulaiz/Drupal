...
class FieldTestHooks
{
    ...
    public function fieldWidgetCompleteFormAlter(array &$field_widget_complete_form, \Drupal\Core\Form\FormStateInterface $form_state, array $context)
    {
    ...
    public function fieldWidgetCompleteTestFieldWidgetMultipleFormAlter(array &$field_widget_complete_form, \Drupal\Core\Form\FormStateInterface $form_state, array $context)
    {
    ...
    public function fieldWidgetCompleteTestFieldWidgetMultipleSingleValueFormAlter(array &$field_widget_complete_form, \Drupal\Core\Form\FormStateInterface $form_state, array $context)
    {
        \_field_test_alter_widget("hook_field_widget_complete_WIDGET_TYPE_form_alter", $field_widget_complete_form, $form_state, $context);
    }
    /**
    ...
    public function entityBundleFieldInfoAlter(&$fields, \Drupal\Core\Entity\EntityTypeInterface $entity_type, $bundle)
    {
        if (($field_name = \Drupal::state()->get('field_test_constraint', \FALSE)) && $entity_type->id() == 'entity_test' && $bundle == 'entity_test' && !empty($fields[$field_name])) {
            ...
            $fields[$field_name]->setPropertyConstraints('value', ['TestField' => ['value' => -2, 'message' => \t('%name does not accept the value @value.', ['%name' => $field_name, '@value' => -2])]]);
    ...
    public function fieldInfoEntityTypeUiDefinitionsAlter(array &$ui_definitions, string $entity_type_id)
    {
        if ($entity_type_id === 'node') {
            $ui_definitions['boolean']['label'] = new \Drupal\Core\StringTranslation\TranslatableMarkup('Boolean (overridden by alter)');
        }
    }
    /**
    ...
    public function entityQueryAlter(\Drupal\Core\Entity\Query\QueryInterface $query) : void
    {
    ...
    public function entityQueryEntityTestMulrevAlter(\Drupal\Core\Entity\Query\QueryInterface $query) : void
    {
    ...
    public function entityQueryTagEntityQueryAlterTagTestAlter(\Drupal\Core\Entity\Query\QueryInterface $query) : void
    {
    ...
    public function entityQueryTagEntityTestMulrevEntityQueryEntityTestMulrevAlterTagTestAlter(\Drupal\Core\Entity\Query\QueryInterface $query) : void
    {
