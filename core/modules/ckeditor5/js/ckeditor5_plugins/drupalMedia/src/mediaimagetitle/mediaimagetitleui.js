...
/**
 * The media image title UI plugin.
 */
export default class MediaImageTitleUi extends Plugin {
  /**
   * @inheritdoc
   */
  static get requires() {
  ...
  /**
   * Creates a button showing the balloon panel for changing the image title
   * and registers it in the editor ComponentFactory.
   */
  _createButton() {
  ...
  /**
   * Creates the {@link module:image/imagetitle/ui/titleformview~TitleFormView}
   * form.
   *
   * @private
   */
  _createForm() {
    ...
    /**
     * A form containing a textarea and buttons, used to change the `title` text value.
     */
    ...
    this.listenTo(this._form, 'submit', () => {
      editor.execute('mediaImageTitle', {
  ...
  _showForm() {
    ...
    if (isDrupalMedia(modelElement)) {
      metadataRepository
        ...
        .then((metadata) => {
          this._form.defaultTitleText = metadata.imageSourceMetadata
            ? metadata.imageSourceMetadata.title
            : '';
        })
        .catch((e) => {
          ...
          // @see module:drupalMedia/mediaimagetitle/mediaimagetitleediting~MediaImageTitleEditing
