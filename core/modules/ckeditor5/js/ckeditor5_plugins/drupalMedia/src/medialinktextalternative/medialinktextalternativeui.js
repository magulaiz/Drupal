...
/* cspell:words linktextalternativeformview */
...
import LinkTextAlternativeFormView from './ui/linktextalternativeformview';

/**
 * The media link text alternative UI plugin.
 */
export default class MediaLinkTextAlternativeUi extends Plugin {
  /**
   * @inheritdoc
   */
  static get requires() {
  ...
  static get pluginName() {
    return 'MediaLinkTextAlternativeUi';
  }
  ...
  _createButton() {
    ...
    editor.ui.componentFactory.add('mediaLinkTextAlternative', (locale) => {
      const command = editor.commands.get('mediaLinkTextAlternative');
      const view = new ButtonView(locale);
      view.set({
        label: Drupal.t('Override media image link text'),
        icon: icons.text,
        tooltip: true,
      });
  ...
  _createForm() {
     ...
     */
    this._form = new LinkTextAlternativeFormView(editor.locale);

    // Render the form so its #element is available for clickOutsideHandler.
    this._form.render();

    this.listenTo(this._form, 'submit', () => {
      editor.execute('mediaLinkTextAlternative', {
  ...
  _showForm() {
    ...
    if (isDrupalMedia(modelElement)) {
      metadataRepository
        ...
        .then((metadata) => {
          this._form.defaultLinkText = metadata.title ? metadata.title : '';
        })
        .catch((e) => {
