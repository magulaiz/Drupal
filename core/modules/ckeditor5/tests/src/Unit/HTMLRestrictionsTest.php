...
class HTMLRestrictionsTest extends UnitTestCase {
  ...
  /**
   * @covers ::notInResolvedSuperset
   * @dataProvider providerNotInResolvedSuperset
   */
  public function testNotInResolvedSuperset(HTMLRestrictions $a, HTMLRestrictions $b, HTMLRestrictions $expected, ?bool $is_diff_equivalent = NULL): void {
    $not = $a->notInResolvedSuperset($b);
    $this->assertEquals($expected, $not);
    $diff = $a->diff($b);
    $this->assertSame($is_diff_equivalent, $diff == $not);
  }

  public function providerNotInResolvedSuperset(): \Generator {
    yield 'empty set' => [
      new HTMLRestrictions([]),
      new HTMLRestrictions([]),
      new HTMLRestrictions([]),
    ...
    yield 'concrete subset of concrete superset: : <p class="foo"> is a subset of <p class>, would work fine with ::diff() too' => [
      new HTMLRestrictions(['p' => ['class' => ['foo' => TRUE]]]),
      new HTMLRestrictions(['p' => ['class' => TRUE]]),
      new HTMLRestrictions([]),
      TRUE,
    ];

    yield 'concrete non-subset of concrete superset: <p class="foo"> is NOT a subset of <div class>' => [
      new HTMLRestrictions(['p' => ['class' => ['foo' => TRUE]]]),
      new HTMLRestrictions(['div' => ['class' => TRUE]]),
      new HTMLRestrictions(['p' => ['class' => ['foo' => TRUE]]]),
      TRUE,
    ];

    yield 'concrete subset of (small) wildcard superset: <p class="foo"> is a subset of <$text-container class>' => [
      new HTMLRestrictions(['p' => ['class' => ['foo' => TRUE]]]),
      new HTMLRestrictions(['$text-container' => ['class' => TRUE]]),
      new HTMLRestrictions([]),
      FALSE,
    ];

    yield 'concrete non-subset of (small) wildcard superset: <img class="foo"> is NOT a subset of <$text-container class>' => [
    ...
    yield 'concrete non-subset of (big) wildcard superset: <drupal-media class="foo"> is NOT a subset of <$any-html5-element class>' => [
      new HTMLRestrictions(['drupal-media' => ['class' => ['foo' => TRUE]]]),
      new HTMLRestrictions(['$any-html5-element' => ['class' => TRUE]]),
      new HTMLRestrictions(['drupal-media' => ['class' => ['foo' => TRUE]]]),
      TRUE,
    ];
