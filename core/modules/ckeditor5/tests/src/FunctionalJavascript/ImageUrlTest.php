...
class ImageUrlTest extends ImageTestBase {
  ...
  public function testAttributeRetentionDuringUpcasting() {
    ...
    foreach ($attributes_to_retain as $attribute_to_retain => $expected_upcast_behavior_when_wrapped_in_block_element) {
      ...
      $img_tag = '<img ' . $attribute_to_retain . ' alt="drupalimage test image" src="/core/misc/druplicon.png" />';
      $test_cases = [
  ...
  public function testImageArbitraryHtml(string $image_type, bool $unrestricted) {
    ...
    $img_tag = '<img data-foo="bar" alt="drupalimage test image" data-entity-type="file" src="/core/misc/druplicon.png" />';
  ...
  public function testLinkability(string $image_type, bool $unrestricted) {
    ...
    // Make the test content have either a block image or an inline image.
    $img_tag = '<img alt="drupalimage test image" data-entity-type="file" src="/core/misc/druplicon.png" />';
    $this->host->body->value .= $image_type === 'block'
      ? $img_tag
      : "<p>$img_tag</p>";
    $this->host->save();
    ...
    $assert_session->elementExists('css', '.ck-content .ck-widget.' . $expected_widget_class . ' > img[src*="druplicon.png"][alt="drupalimage test image"]');
    ...
    $assert_session->elementExists('css', $image_type === 'inline'
      ? '.ck-content a[href*="//www.drupal.org/association"] .ck-widget.' . $expected_widget_class . ' > img[src*="druplicon.png"][alt="drupalimage test image"]'
      : '.ck-content .ck-widget.' . $expected_widget_class . ' a[href*="//www.drupal.org/association"] > img[src*="druplicon.png"][alt="drupalimage test image"]'
    );
    ...
    $assert_session->elementExists('css', $unrestricted
      ? 'a[href="http://www.drupal.org/association"].trusted img[src*="druplicon.png"]'
      : 'a[href="http://www.drupal.org/association"] img[src*="druplicon.png"]');
  ...
  public function testAltTextRequired(bool $unrestricted) {
    ...
    // Make the test content have a block image and an inline image.
    $img_tag = '<img src="/core/misc/druplicon.png" width="500" />';
    ...
    // Ensure that alt form is opened after image insert.
    $this->drupalGet($this->host->toUrl('edit-form'));
    $this->waitForEditor();
    $this->pressEditorButton('Insert image');
    $panel = $page->find('css', '.ck-dropdown__panel.ck-image-insert__panel');
    $src_input = $panel->find('css', 'input[type=text]');
    $src_input->setValue('/core/misc/druplicon.png');
    $panel->find('xpath', "//button[span[text()='Insert']]")->click();
  ...
  public function testAlignment(string $image_type): void {
    ...
    $page = $this->getSession()->getPage();
    // Make the test content have either a block image or an inline image.
    $img_tag = '<img alt="drupalimage test image" src="/core/misc/druplicon.png" />';
    $this->host->body->value .= $image_type === 'block'
      ? $img_tag
      : "<p>$img_tag</p>";
    $this->host->save();
  ...
  public function testWidth(string $width): void {
    ...
    $this->host->body->value = sprintf('<img data-foo="bar" alt="drupalimage test image" src="/core/misc/druplicon.png" width="%s" />', $width);
    $this->host->save();
  ...
  public function testImageCaption() {
    ...
    $img_tag = '<img alt="drupalimage test image" data-caption="Alpacas &lt;em&gt;are&lt;/em&gt; cute" foo="bar" src="/core/misc/druplicon.png">';
    ...
    $this->assertEquals('<img src="/core/misc/druplicon.png" alt="drupalimage test image" data-caption="Alpacas &lt;em&gt;are&lt;/em&gt; cute">', Node::load(1)->get('body')->value);
    $assert_session->elementExists('xpath', '//figure/img[@src="/core/misc/druplicon.png" and not(@data-caption)]');
