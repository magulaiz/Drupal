...
class CKEditor5PluginManager extends DefaultPluginManager implements CKEditor5PluginManagerInterface {
  ...
  public function getEnabledDefinitions(EditorInterface $editor): array {
    ...
    if (!isset($definitions['ckeditor5_arbitraryHtmlSupport'])) {
      ...
      if ($restrictions->getWildcardSubset()->hasNoAllowedElements()) {
  ...
  public function getProvidedElements(array $plugin_ids = [], EditorInterface $editor = NULL, bool $resolve_wildcards = TRUE): array {
    ...
    foreach ($plugins as $id => $definition) {
      ...
      if (is_a($definition->getClass(), CKEditor5PluginElementsSubsetInterface::class, TRUE)) {
        ...
        // supported, this is a little lie to convey that this plugin is capable
        // of supporting any HTML tag … but which ones are actually supported
