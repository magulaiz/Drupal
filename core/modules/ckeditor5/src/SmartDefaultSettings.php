  ...
  private function createSettingsFromCKEditor4(array $ckeditor4_settings, array $enabled_ckeditor4_plugins, HTMLRestrictions $text_format_html_restrictions): array {
      ...
      foreach ($row as $group) {
        ...
        foreach ($group['items'] as $cke4_button) {
          try {
            ...
            $warning = $this->t('The CKEditor 4 button %button does not have a known upgrade path. If it allowed editing markup, then you can do so now through the Source Editing functionality.', [
              '%button' => $cke4_button,
            ]);
    ...
    foreach ($enabled_ckeditor4_plugins_with_settings as $cke4_plugin_id => $cke4_plugin_settings) {
      try {
        ...
        $warning = $this->t('The %cke4_plugin_id plugin settings do not have a known upgrade path.', [
