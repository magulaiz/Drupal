...
class RequirementHooks {
  ...
  /**
   * Runtime and Update requirements for system module.
   *
   * Shared implementation to avoid code duplication.
   *
   * @param string $phase
   *   The legacy requirement phase.
   *
   * @return array
   */
  protected function requirements(string $phase): array {
    ...
    if ($phase == 'runtime') {
      ...
      if (!empty($experimental_modules)) {
        $requirements['experimental_modules'] = [
          ...
          'value' => $this->t('Experimental modules found: %module_list. <a href=":url">Experimental modules</a> are provided for testing purposes only. Use at your own risk.', ['%module_list' => implode(', ', $experimental_modules), ':url' => 'https://www.drupal.org/core/experimental']),
    ...
    if (version_compare($phpversion, $minimum_supported_php) < 0) {
      $requirements['php']['description'] = $this->t('Your PHP installation is too old. Drupal requires at least PHP %version. It is recommended to upgrade to PHP version %recommended or higher for the best ongoing support. See <a href="http://php.net/supported-versions.php">PHP\'s version support documentation</a> and the <a href=":php_requirements">Drupal PHP requirements</a> page for more information.',
        [
    ...
    if ($phase != 'update') {
      ...
      try {
        ...
        if ($open_basedir_blocks_urandom) {
          $requirements['php_random_bytes']['description'] = $this->t('Drupal is unable to generate highly randomized numbers, which means certain security features like password reset URLs are not as secure as they should be. Instead, only a slow, less-secure fallback generator is available. The most likely cause is that open_basedir restrictions are in effect and /dev/urandom is not on the whitelist. See the <a href=":drupal-php">system requirements</a> page for more information. %exception_message', $args);
        }
        else {
      ...
      foreach ($htaccessWriter->defaultProtectedDirs() as $protected_dir) {
        ...
        if (!file_exists($htaccess_file) || !($contents = @file_get_contents($htaccess_file)) || !str_contains($contents, 'Drupal_Security_Do_Not_Remove_See_SA_2013_003')) {
          ...
          $requirements[$htaccess_file] = [
            ...
            'description' => $this->t('See <a href=":url">@url</a> for information about the recommended .htaccess file which should be added to the %directory directory to help protect against arbitrary code execution.', [':url' => $url, '@url' => $url, '%directory' => $protected_dir->getPath()]),
          ];
        }
      }
    }
    ...
    if (empty($config_sync_directory)) {
      $requirements['config sync directory'] = [
        ...
        'description' => $this->t("Your %file file must define the %setting setting as a string containing the directory in which configuration files can be found.", ['%file' => $site_path . '/settings.php', '%setting' => "\$settings['config_sync_directory']"]),
        'severity' => REQUIREMENT_ERROR,
      ];
    }
      ...
      foreach ($files as $extension_name => $file) {
        ...
        foreach ($file->requires as $requirement) {
          ...
          if (!$requirement->isCompatible($version)) {
            $requirements["$extension_name-$required_module"] = [
              'title' => $this->t('Unresolved dependency'),
              'description' => $this->t('@name requires this module and version. Currently using @required_name version @version', ['@name' => $name, '@required_name' => $required_name, '@version' => $version]),
              'value' => $this->t('@required_name (Version @compatibility required)', ['@required_name' => $required_name, '@compatibility' => $requirement->getConstraintString()]),
              'severity' => REQUIREMENT_ERROR,
            ];
            continue;
          }
    ...
    if ($phase == 'runtime') {
      ...
      if (!\Drupal::moduleHandler()->moduleExists('update')) {
        $requirements['update status'] = [
          'value' => $this->t('Not enabled'),
          'severity' => REQUIREMENT_WARNING,
          'description' => $this->t('Update notifications are not enabled. It is <strong>highly recommended</strong> that you install the Update Manager module from the <a href=":module">module administration page</a> in order to stay up-to-date on new releases. For more information, <a href=":update">Update status handbook page</a>.', [
        ...
        if (str_contains($autoload, 'src/Driver/Database/')) {
          ...
          if (!in_array('enable_provider_database_driver', array_keys($pending_updates['system']['pending'] ?? []), TRUE)) {
            ...
            $requirements['database_driver_provided_by_module'] = [
              ...
              'description' => $this->t('The current database driver is provided by the module: %module. The module is currently not installed. You should immediately <a href=":install">install</a> the module.', ['%module' => $provider, ':install' => Url::fromRoute('system.modules_list')->toString()]),
              'severity' => REQUIREMENT_ERROR,
            ];
          }
        }
