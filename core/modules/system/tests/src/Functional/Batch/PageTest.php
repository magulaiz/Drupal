...
class PageTest extends BrowserTestBase {
  ...
  public function testBatchProgressPageTheme() {
    ...
    // page.
    $this->assertEquals(['claro'], \Drupal::service('batch_test.batch_test_stack')->batchTestStack(), 'A progressive batch correctly uses the theme of the page that started the batch.');
  }
  ...
  public function testBatchProgressPageTitle() {
    ...
    $this->assertEquals(['Batch Test'], \Drupal::service('batch_test.batch_test_stack')->batchTestStack(), 'The batch title is shown on the batch page.');
