...
class UrlTest extends BrowserTestBase {
  ...
  public function testActiveLinkAttributes() {
    ...
    // Test that a link generated by the link generator to the current page
    // without a query string when the current page has a query string is
    // marked active.
    $this->assertSession()->elementExists('xpath', $this->assertSession()->buildXPathQuery(
      '//a[@href = :href and contains(@class, :class)]', [
        ':href' => Url::fromRoute('common_test.l_active_class', [], $options_no_query)->toString(),
        ':class' => 'is-active',
      ]
    ));
    // Test that a link generated by the link generator without a query arg is
    // marked active on a page with a query arg.
    $this->assertSession()->elementExists('xpath', $this->assertSession()->buildXPathQuery(
      '//a[@href = :href and contains(@class, :class) and not(@data-drupal-link-query)]', [
        ':href' => Url::fromRoute('common_test.l_active_class', [], $options_no_query)->toString(),
        ':class' => 'is-active',
      ]
    ));

    $this->drupalGet($path, ['query' => ['arg' => 'irrelevant']]);

    // Test that a link generated by the link generator to the current page
    // with a query string that does not match the current page's query string
    // is not marked active.
    $this->assertSession()->elementExists('xpath', $this->assertSession()->buildXPathQuery(
      '//a[@href = :href and not(contains(@class, :class))]', [
        ':href' => Url::fromRoute('common_test.l_active_class', [], $options_query)->toString(),
        ':class' => 'is-active',
      ]
    ));
