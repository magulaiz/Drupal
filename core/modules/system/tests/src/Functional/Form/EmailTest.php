...
class EmailTest extends BrowserTestBase {
  ...
  public function testFormEmail() {
    ...
    $this->assertSession()->pageTextContains("The email addresses invalid-second, invalid-second is not valid.");
    ...
    $this->drupalGet('form-test/email');
    $this->submitForm($edit, 'Submit');
    $this->assertSession()->pageTextContains('The email address invalid-required is not valid.');
    $this->assertSession()->pageTextContains('The email address invalid-multiple is not valid.');
    ...
    $this->drupalGet('form-test/email');
    $this->submitForm($edit, 'Submit');
    $this->assertSession()->pageTextContains('The email address example@drupal.org,example2@drupal.org is not valid.');
