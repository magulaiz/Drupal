...
class DelayCacheTagsInvalidationHooks
{
    ...
    public function entityTestInsert(\Drupal\entity_test\Entity\EntityTest $entity)
    {
        ...
        \Drupal::cache()->set(__FUNCTION__ . '__during_transaction_foobar', 'something', \Drupal\Core\Cache\Cache::PERMANENT, ['foobar']);
        \Drupal::cache()->set(__FUNCTION__ . '__during_transaction_entity_test_list', 'something', \Drupal\Core\Cache\Cache::PERMANENT, ['entity_test_list']);
        // Trigger a nested entity save and hence a nested transaction.
        \Drupal\user\Entity\User::create(['name' => 'john doe', 'status' => 1])->save();
    }
    /**
     * Implements hook_ENTITY_TYPE_insert().
     */
    #[Hook('user_insert')]
    public function userInsert(\Drupal\user\UserInterface $entity)
    {
