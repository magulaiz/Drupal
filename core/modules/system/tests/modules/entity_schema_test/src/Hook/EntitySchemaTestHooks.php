...
class EntitySchemaTestHooks
{
    ...
    public function entityBaseFieldInfo(\Drupal\Core\Entity\EntityTypeInterface $entity_type)
    {
        if ($entity_type->id() == 'entity_test_update') {
            $definitions['custom_base_field'] = \Drupal\Core\Field\BaseFieldDefinition::create('string')->setName('custom_base_field')->setLabel(\t('A custom base field'));
            if (\Drupal::state()->get('entity_schema_update')) {
                $definitions += \Drupal\entity_test\Entity\EntityTestMulRev::baseFieldDefinitions($entity_type);
                // And add a revision log.
                $definitions['revision_log'] = \Drupal\Core\Field\BaseFieldDefinition::create('string_long')->setLabel(\t('Revision log message'))->setDescription(\t('The log entry explaining the changes in this revision.'))->setRevisionable(\TRUE);
            }
            return $definitions;
        }
    }
    ...
    {
        if ($entity_type->id() == 'entity_test_update') {
            $definitions['custom_bundle_field'] = \Drupal\entity_test\FieldStorageDefinition::create('string')->setName('custom_bundle_field')->setLabel(\t('A custom bundle field'))->setRevisionable(\TRUE)->setTargetEntityTypeId($entity_type->id());
            return $definitions;
        }
    }
    /**
    ...
    public function entityBundleFieldInfo(\Drupal\Core\Entity\EntityTypeInterface $entity_type, $bundle)
    {
        if ($entity_type->id() == 'entity_test_update' && $bundle == 'custom') {
            ...
            $definitions[$custom_bundle_field_storage->getName()] = \Drupal\Core\Field\FieldDefinition::createFromFieldStorageDefinition($custom_bundle_field_storage);
            return $definitions;
        }
    }
    /**
