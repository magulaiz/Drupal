...
class SessionExistsCacheContextTestHooks {
  ...
  public function pageTop(array &$page_top) {
    ...
    $page_top['session_exists_cache_context_test'] = ['label' => ['#markup' => '<p>' . ($session_exists ? 'Session exists!' : 'Session does not exist!') . '</p>'], 'cache_context_value' => ['#markup' => '<code>[session.exists]=' . \Drupal::service('cache_context.session.exists')->getContext() . '</code>']];
    $request = \Drupal::request();
