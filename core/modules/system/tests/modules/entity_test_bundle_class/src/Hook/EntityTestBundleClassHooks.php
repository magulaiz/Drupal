...
class EntityTestBundleClassHooks
{
    ...
    public function entityBundleInfoAlter(&$bundles)
    {
        if (!empty($bundles['entity_test']['bundle_class'])) {
            $bundles['entity_test']['bundle_class']['class'] = \Drupal\entity_test_bundle_class\Entity\EntityTestBundleClass::class;
        }
        if (\Drupal::state()->get('entity_test_bundle_class_enable_ambiguous_entity_types', \FALSE)) {
            $bundles['entity_test']['bundle_class_2']['class'] = \Drupal\entity_test_bundle_class\Entity\EntityTestBundleClass::class;
            $bundles['entity_test']['entity_test_no_label']['class'] = \Drupal\entity_test_bundle_class\Entity\EntityTestAmbiguousBundleClass::class;
            $bundles['entity_test_no_label']['entity_test_no_label']['class'] = \Drupal\entity_test_bundle_class\Entity\EntityTestAmbiguousBundleClass::class;
        }
        if (\Drupal::state()->get('entity_test_bundle_class_non_inheriting', \FALSE)) {
            $bundles['entity_test']['bundle_class']['class'] = \Drupal\entity_test_bundle_class\Entity\NonInheritingBundleClass::class;
        }
        if (\Drupal::state()->get('entity_test_bundle_class_enable_user_class', \FALSE)) {
            $bundles['user']['user']['class'] = \Drupal\entity_test_bundle_class\Entity\EntityTestUserClass::class;
        }
        ...
        $bundles['shared_type']['bundle_a'] = ['label' => 'Bundle A', 'class' => \Drupal\entity_test_bundle_class\Entity\SharedEntityTestBundleClassA::class];
        $bundles['shared_type']['bundle_b'] = ['label' => 'Bundle B', 'class' => \Drupal\entity_test_bundle_class\Entity\SharedEntityTestBundleClassB::class];
    }
    /**
    ...
    public function entityTypeAlter(&$entity_types)
    {
        ...
        if (\Drupal::state()->get('entity_test_bundle_class_override_base_class', \FALSE) && isset($entity_types['entity_test'])) {
            $entity_types['entity_test']->setClass(\Drupal\entity_test_bundle_class\Entity\EntityTestVariant::class);
        }
    }
    /**
    ...
    public function entityTypeBuild(array &$entity_types) : void
    {
        ...
        $entity_types['shared_type']->setClass(\Drupal\entity_test\Entity\EntityTest::class);
    }
}
