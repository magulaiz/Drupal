...
class EntityTestHooks
{
    ...
    public function entityBaseFieldInfo(\Drupal\Core\Entity\EntityTypeInterface $entity_type)
    {
        ...
        if ($entity_type->id() === 'entity_test' && \Drupal::state()->get('entity_test.internal_field')) {
            $fields['internal_string_field'] = \Drupal\Core\Field\BaseFieldDefinition::create('string')->setLabel('Internal field')->setInternal(\TRUE);
        }
        if ($entity_type->id() === 'entity_test_mul' && \Drupal::state()->get('entity_test.required_default_field')) {
            $fields['required_default_field'] = \Drupal\Core\Field\BaseFieldDefinition::create('string')->setLabel('Required field with default value')->setRequired(\TRUE)->setDefaultValue('this is a default value');
        }
        if ($entity_type->id() === 'entity_test_mul' && \Drupal::state()->get('entity_test.required_multi_default_field')) {
            $fields['required_multi_default_field'] = \Drupal\Core\Field\BaseFieldDefinition::create('string')->setLabel('Required field with default value')->setRequired(\TRUE)->setCardinality(\Drupal\Core\Field\FieldStorageDefinitionInterface::CARDINALITY_UNLIMITED)->setDefaultValue([['value' => 'this is the first default field item'], ['value' => 'this is the second default value'], ['value' => 'you get the idea...']]);
        }
        if ($entity_type->id() == 'entity_test_mulrev' && \Drupal::state()->get('entity_test.field_test_item')) {
            $fields['field_test_item'] = \Drupal\Core\Field\BaseFieldDefinition::create('field_test')->setLabel(\t('Field test'))->setDescription(\t('A field test.'))->setRevisionable(\TRUE)->setTranslatable(\TRUE);
        }
        if ($entity_type->id() == 'entity_test_mulrev' && \Drupal::state()->get('entity_test.multi_column')) {
            $fields['description'] = \Drupal\Core\Field\BaseFieldDefinition::create('shape')->setLabel(\t('Some custom description'))->setTranslatable(\TRUE);
        }
        return $fields;
    }
    /**
    ...
    public function entityBaseFieldInfoAlter(&$fields, \Drupal\Core\Entity\EntityTypeInterface $entity_type)
    {
        ...
        if ($entity_type->id() == 'entity_test' && $state->get('entity_test.db_updates.entity_definition_updates') == 8001) {
            $fields['user_id']->setCardinality(\Drupal\Core\Field\FieldStorageDefinitionInterface::CARDINALITY_UNLIMITED);
        }
    }
    /**
    ...
    public function entityViewModeInfoAlter(&$view_modes)
    {
        ...
        foreach ($entity_info as $entity_type => $info) {
            if ($entity_info[$entity_type]->getProvider() == 'entity_test' && !isset($view_modes[$entity_type])) {
                $view_modes[$entity_type] = ['full' => ['label' => \t('Full object'), 'status' => \TRUE, 'cache' => \TRUE], 'teaser' => ['label' => \t('Teaser'), 'status' => \TRUE, 'cache' => \TRUE]];
            }
        }
    }
    /**
    ...
    public function entityFormModeInfoAlter(&$form_modes)
    {
        ...
        foreach ($entity_info as $entity_type => $info) {
            if ($entity_info[$entity_type]->getProvider() == 'entity_test') {
                $form_modes[$entity_type]['compact'] = ['label' => \t('Compact version'), 'status' => \TRUE];
            }
        }
    }
    /**
    ...
    public function entityTestViewModeAlter(string &$view_mode, \Drupal\Core\Entity\EntityInterface $entity) : void
    {
    ...
    {
        $extra['entity_test']['bundle_with_extra_fields'] = ['display' => [
            ...
            'display_extra_field' => ['label' => \t('Display extra field'), 'description' => \t('An extra field on the display side.'), 'weight' => 5, 'visible' => \TRUE],
            'display_extra_field_hidden' => ['label' => \t('Display extra field (hidden)'), 'description' => \t('An extra field on the display side, hidden by default.'), 'visible' => \FALSE],
        ]];
        return $extra;
    }
    /**
    ...
    public function formEntityTestFormAlter(&$form)
    {
    ...
    public function entityFieldAccess($operation, \Drupal\Core\Field\FieldDefinitionInterface $field_definition, \Drupal\Core\Session\AccountInterface $account, ?\Drupal\Core\Field\FieldItemListInterface $items = \NULL)
    {
        if ($field_definition->getName() == 'field_test_text') {
            if ($items) {
                if ($items->value == 'no access value') {
                    ...
                    return \Drupal\Core\Access\AccessResult::allowed()->addCacheableDependency($items->getEntity())->addCacheTags(['entity_test_access:field_test_text']);
        ...
        if ($field = \Drupal::state()->get('views_field_access_test-field')) {
            if ($field_definition->getName() === $field) {
                $result = \Drupal\Core\Access\AccessResult::allowedIfHasPermission($account, 'view test entity field');
                // For test purposes we want to actively deny access.
                if ($result->isNeutral()) {
                    $result = \Drupal\Core\Access\AccessResult::forbidden();
                }
                return $result;
            }
        }
    ...
    public function entityFieldAccessAlter(array &$grants, array $context)
    {
        if ($context['field_definition']->getName() == 'field_test_text' && $context['items']->value == 'access alter value') {
            $grants[':default'] = \Drupal\Core\Access\AccessResult::forbidden()->inheritCacheability($grants[':default'])->addCacheableDependency($context['items']->getEntity());
        }
    }
    /**
    ...
    public function entityFormModeAlter(&$form_mode, \Drupal\Core\Entity\EntityInterface $entity)
    {
    ...
    public function entityFormDisplayAlter(\Drupal\Core\Entity\Entity\EntityFormDisplay $form_display, $context)
    {
    ...
    public function entityOperationAlter(array &$operations, \Drupal\Core\Entity\EntityInterface $entity)
    {
        ...
        if (\in_array($entity->getEntityTypeId(), $valid_entity_type_ids)) {
            if (\Drupal::service('router.route_provider')->getRouteByName("entity.{$entity->getEntityTypeId()}.test_operation")) {
                $operations['test_operation'] = ['title' => new \Drupal\Component\Render\FormattableMarkup('Test Operation: @label', ['@label' => $entity->label()]), 'url' => \Drupal\Core\Url::fromRoute("entity.{$entity->getEntityTypeId()}.test_operation", [$entity->getEntityTypeId() => $entity->id()]), 'weight' => 50];
            }
        }
    }
    /**
    ...
    public function entityTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulTranslationDelete(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulChangedTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulChangedTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulChangedTranslationDelete(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevTranslationDelete(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevChangedTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevChangedTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulrevChangedTranslationDelete(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulLangcodeKeyTranslationCreate(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulLangcodeKeyTranslationInsert(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityTestMulLangcodeKeyTranslationDelete(\Drupal\Core\Entity\EntityInterface $translation)
    {
    ...
    public function entityRevisionCreate(\Drupal\Core\Entity\EntityInterface $new_revision, \Drupal\Core\Entity\EntityInterface $entity, $keep_untranslatable_fields)
    {
    ...
    public function entityTestMulrevRevisionCreate(\Drupal\Core\Entity\EntityInterface $new_revision, \Drupal\Core\Entity\EntityInterface $entity, $keep_untranslatable_fields)
    {
    ...
    public function entityAccess(\Drupal\Core\Entity\EntityInterface $entity, $operation, \Drupal\Core\Session\AccountInterface $account)
    {
        ...
        return \Drupal\Core\Access\AccessResult::allowedIf($condition)->setCacheMaxAge(0);
    }
    /**
     * Implements hook_ENTITY_TYPE_access() for 'entity_test'.
     */
    #[Hook('entity_test_access')]
    public function entityTestAccess(\Drupal\Core\Entity\EntityInterface $entity, $operation, \Drupal\Core\Session\AccountInterface $account)
    {
    ...
    public function entityCreateAccess(\Drupal\Core\Session\AccountInterface $account, $context, $entity_bundle)
    {
    ...
    public function entityTestCreateAccess(\Drupal\Core\Session\AccountInterface $account, $context, $entity_bundle)
    {
    ...
    public function queryEntityTestAccessAlter(\Drupal\Core\Database\Query\AlterableInterface $query)
    {
    ...
    public function entityTestFormModeAlter(&$form_mode, \Drupal\Core\Entity\EntityInterface $entity) : void
    {
