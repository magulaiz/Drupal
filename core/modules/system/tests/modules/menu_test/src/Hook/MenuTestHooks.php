...
class MenuTestHooks
{
    ...
    public function menuLinksDiscoveredAlter(&$links)
    {
        ...
        $links['menu_test.custom'] = ['title' => 'Custom link', 'route_name' => 'menu_test.custom', 'description' => 'Custom link used to check the integrity of manually added menu links.', 'parent' => 'menu_test'];
    }
    /**
    ...
    public function menuLocalTasksAlter(&$data, $route_name, \Drupal\Core\Cache\RefinableCacheableDependencyInterface &$cacheability)
    {
        if (\in_array($route_name, ['menu_test.tasks_default'])) {
            $data['tabs'][0]['foo'] = ['#theme' => 'menu_local_task', '#link' => ['title' => "Task 1 <script>alert('Welcome to the jungle!')</script>", 'url' => \Drupal\Core\Url::fromRoute('menu_test.router_test1', ['bar' => '1'])], '#weight' => 10];
            $data['tabs'][0]['bar'] = ['#theme' => 'menu_local_task', '#link' => ['title' => 'Task 2', 'url' => \Drupal\Core\Url::fromRoute('menu_test.router_test2', ['bar' => '2'])], '#weight' => 20];
        }
