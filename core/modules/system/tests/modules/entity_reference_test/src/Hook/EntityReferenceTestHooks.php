...
class EntityReferenceTestHooks
{
    ...
    public function entityBaseFieldInfo(\Drupal\Core\Entity\EntityTypeInterface $entity_type)
    {
        ...
        if ($entity_type->id() === 'entity_test') {
            $fields['user_role'] = \Drupal\Core\Field\BaseFieldDefinition::create('entity_reference')->setLabel(\t('User role'))->setDescription(\t('The role of the associated user.'))->setSetting('target_type', 'user_role')->setSetting('handler', 'default');
        }
        return $fields;
    }
    /**
