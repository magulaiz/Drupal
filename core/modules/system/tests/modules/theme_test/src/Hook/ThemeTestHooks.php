...
class ThemeTestHooks
{
    ...
    public function theme($existing, $type, $theme, $path)
    {
        ...
        $items['theme_test_deprecations_preprocess'] = ['variables' => ['foo' => '', 'bar' => '', 'gaz' => '', 'set_var' => '', 'for_var' => '', 'contents' => []]];
        $items['theme_test_deprecations_child'] = ['variables' => ['foo' => '', 'bar' => '', 'gaz' => '']];
        $items['theme_test_deprecations_hook_theme'] = ['variables' => ['foo' => '', 'bar' => '', 'deprecations' => ['foo' => "'foo' is deprecated in drupal:X.0.0 and is removed from drupal:Y.0.0. Use 'new_foo' instead. See https://www.example.com.", 'bar' => "'bar' is deprecated in drupal:X.0.0 and is removed from drupal:Y.0.0. Use 'new_bar' instead. See https://www.example.com."]]];
        return $items;
    }
    /**
    ...
    public function libraryInfoAlter(array &$libraries, string $extension) : void
    {
        ...
        if (\is_array($info)) {
            $libraries = \Drupal\Component\Utility\NestedArray::mergeDeep($libraries, $info);
        }
    }
}
