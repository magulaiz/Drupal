...
class MenuLinkContentStorage extends SqlContentEntityStorage implements MenuLinkContentStorageInterface {
  ...
  public function getMenuLinkIdsWithPendingRevisions() {
    ...
    $query->join($this->getRevisionTable(), 'mlr', $this->database->condition('AND')->compare("mlfr.$revision_field", "mlr.$revision_field")->condition("mlr.$revision_default_field", 0));
    ...
    $query->join($inner_select, 'mr', $this->database->condition('AND')->compare("mlfr.$revision_field", "mr.$revision_field")->compare("mlfr.$langcode_field", "mr.$langcode_field"));
