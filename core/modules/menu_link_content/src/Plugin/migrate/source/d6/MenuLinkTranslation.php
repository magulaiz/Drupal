...
class MenuLinkTranslation extends MenuLink {
  ...
  public function query() {
    ...
    $query->leftJoin(static::I18N_STRING_TABLE, 'i18n', $this->database->condition('AND')->where('CAST([ml].[mlid] AS CHAR(255)) = [i18n].[objectid]'));
    $query->addField('i18n', 'lid');
    $query->addField('i18n', 'property');

    // Add in the translation for the property.
    $query->innerJoin('locales_target', 'lt', $this->database->condition('AND')->compare('i18n.lid', 'lt.lid'));
