...
class MenuLinkContentForm extends ContentEntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    if ($this->entity->isNew()) {
      $form['menu_parent_fieldset']['menu_parent'] = $this->menuParentSelector->parentSelectElement($default, $id, [
        $menu_id => $menu->label(),
      ]);
      $form['menu_parent_fieldset']['#type'] = 'container';
      $form['menu_parent_fieldset']['menu_parent']['#weight'] = 10;
      $form['menu_parent_fieldset']['menu_parent']['#title'] = $this->t('Parent link');
      $form['menu_parent_fieldset']['menu_parent']['#description'] = $this->t('The maximum depth for a link and all its children is fixed. Some menu links may not be available as parents if selecting them would exceed this limit.');
      $form['menu_parent_fieldset']['menu_parent']['#attributes']['class'][] = 'menu-title-select';
    }
    else {
      $form += $this->menuParentSelector->parentSelectElement($form_state->getValue('menu') ?: $default, '', NULL, $form_state->getValue('menu') ?: $menu_id . ':');
      $form['menu_parent_fieldset']['menu_parent_wrapper']['menu_parent'] += $this->menuParentSelector->parentSelectElement($form_state->getValue('menu') ?: $default, $id, $form_state->getValue('menus') ?: [$menu_id => $menu->label()]);
      ...
      $form['menu_parent_fieldset']['menu_parent_wrapper']['menu_parent']['#description'] = $this->t('The maximum depth for a link and all its children is fixed. Some menu links may not be available as parents if selecting them would exceed this limit.');
