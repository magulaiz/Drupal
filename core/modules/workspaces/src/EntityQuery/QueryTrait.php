...
trait QueryTrait {
  ...
  public function prepare() {
    ...
    if ($this->workspaceManager->hasActiveWorkspace() && $this->workspaceManager->isEntityTypeSupported($this->entityType)) {
      ...
      $this->sqlQuery->leftJoin('workspace_association', 'workspace_association', $this->sqlQuery->getConnection()->condition('AND')->condition("%alias.target_entity_type_id", $this->entityTypeId)->compare("%alias.target_entity_id", "base_table.$id_field")->condition("%alias.workspace", $active_workspace->id()));
    }
