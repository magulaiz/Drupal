...
class ViewsHooks {
  ...
  public function hookInfo() {
    $hooks = [];
    $hooks += array_fill_keys(['views_data', 'views_data_alter', 'views_analyze', 'views_invalidate_cache'], ['group' => 'views']);
    // Register a views_plugins alter hook for all plugin types.
    foreach (ViewExecutable::getPluginTypes() as $type) {
      $hooks['views_plugins_' . $type . '_alter'] = ['group' => 'views'];
    }
    ...
    $hooks['field_views_data'] = ['group' => 'views'];
    $hooks['field_views_data_alter'] = ['group' => 'views'];
    return $hooks;
  }
