...
class RowRenderCacheTest extends ViewsKernelTestBase {
  ...
  protected function doTestRenderedOutput(AccountInterface $account, $check_cache = FALSE): void {
    ...
    foreach ($nodes as $node) {
      ...
      $this->assertSame($expected, (string) $output);
      $expected = $access ? preg_replace('/\s+/', '', '
        <div class="dropbutton-wrapper" data-drupal-ajax-container><div class="dropbutton-widget"><ul class="dropbutton">
      ...
      $output = preg_replace('/\s+/', '', (string) $view->style_plugin->getField($index, 'operations'));
      $this->assertSame(trim($expected), (string) trim($output));
