...
class RowRenderCacheTest extends ViewsKernelTestBase {
  ...
  protected function doTestRenderedOutput(AccountInterface $account, $check_cache = FALSE): void {
    ...
    foreach ($nodes as $node) {
      ...
      $this->assertSame($expected, (string) $output);
      $expected = $access ? '  <div class="dropbutton-wrapper" data-drupal-ajax-container>' . PHP_EOL . '    <div class="dropbutton-widget"><ul class="dropbutton"><li><a href="' . $node_url . '/edit?destination=/" aria-label="Edit ' . $node->label() . '" hreflang="en">Edit</a></li>' .
