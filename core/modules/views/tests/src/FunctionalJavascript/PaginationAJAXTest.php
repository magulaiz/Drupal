...
class PaginationAJAXTest extends WebDriverTestBase {
  ...
  public function testBasicPagination(): void {
    ...
    $this->assertEquals('?order=changed&sort=asc&status=All&type=All&title=&items_per_page=5&page=2', $link->getAttribute('href'));
  ...
  public function testDefaultFilterPagination(): void {
    ...
    // Test that no unwanted parameters are added to the URL.
    $this->assertEquals('?order=changed&sort=asc&status=All&type=All&title=default_value&items_per_page=5&page=0', $link->getAttribute('href'));
