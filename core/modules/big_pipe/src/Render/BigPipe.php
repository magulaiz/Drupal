...
class BigPipe {
  ...
  public function __construct(RendererInterface $renderer, SessionInterface $session, RequestStack $request_stack, HttpKernelInterface $http_kernel, EventDispatcherInterface $event_dispatcher, ConfigFactoryInterface $config_factory, protected ?MessengerInterface $messenger = NULL, protected ?RequestContext $requestContext = NULL, protected ?LoggerInterface $logger = NULL) {
    ...
    if (!isset($this->requestContext)) {
      @trigger_error('Calling ' . __CLASS__ . '::_construct() without the $requestContext argument is deprecated in drupal:10.4.0 and it will be required in drupal:11.0.0. See https://www.drupal.org/node/3304746', E_USER_DEPRECATED);
      $this->requestContext = \Drupal::service('router.request_context');
    }
    if (!isset($this->logger)) {
      @trigger_error('Calling ' . __CLASS__ . '::_construct() without the $logger argument is deprecated in drupal:10.4.0 and it will be required in drupal:11.0.0. See https://www.drupal.org/node/3304746', E_USER_DEPRECATED);
