  ...
  public function help($route_name, RouteMatchInterface $route_match): string {
    ...
    switch ($route_name) {
      ...
      case 'entity.path_alias.collection':
        $output = '<p>' . t("An alias defines a different name for an existing URL path - for example, the alias 'about' for the URL path 'node/1'. A URL path can have multiple aliases.") . '</p>';
        break;

      case 'entity.path_alias.add_form':
        $output = '<p>' . t('Enter the path you wish to create the alias for, followed by the name of the new alias.') . '</p>';
        break;
    }
