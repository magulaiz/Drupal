...
final class AdminStatusCheckMessages implements ContainerInjectionInterface {
  ...
  public static function create(ContainerInterface $container): self {
    return new static(
      $container->get('auto_updates.status_checker'),
  ...
  public function displayAdminPageMessages(): void {
    ...
    if ($this->statusChecker->getResults() === NULL) {
      $checker_url = Url::fromRoute('auto_updates.status_check')->setOption('query', $this->getDestinationArray());
  ...
  protected function displayResultsOnCurrentPage(): bool {
    ...
    if ($this->adminContext->isAdminRoute() && $this->currentUser->hasPermission('administer site configuration')) {
      ...
      if ($route) {
        if ($route->hasOption('_auto_updates_readiness_messages')) {
          @trigger_error('The _auto_updates_readiness_messages route option is deprecated in auto_updates:8.x-2.5 and will be removed in auto_updates:3.0.0. Use _auto_updates_status_messages route option instead. See https://www.drupal.org/node/3316086.', E_USER_DEPRECATED);
          $route->setOption('_auto_updates_status_messages', $route->getOption('_auto_updates_readiness_messages'));
        }
        return $route->getOption('_auto_updates_status_messages') !== 'skip';
      }
    }
    return FALSE;
  }
