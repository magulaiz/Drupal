...
final class StatusCheckRequirements implements ContainerInjectionInterface {
  ...
  private function getMethod(): string {
    return $this->configFactory->get('auto_updates.settings')
  ...
  public function getRequirements(): array {
    ...
    if (is_null($results) && $this->getMethod() === 'console') {
      $requirements['auto_updates_status_check_console_command_not_run'] = [
    ...
    if (empty($results)) {
      $requirements['auto_updates_status_check'] = [
        'title' => $this->t('Update readiness checks'),
      ...
      if ($run_link) {
        $requirements['auto_updates_status_check']['description'] = $run_link;
      }
    }
    else {
      foreach ([SystemManager::REQUIREMENT_WARNING, SystemManager::REQUIREMENT_ERROR] as $severity) {
        if ($requirement = $this->createRequirementForSeverity($severity)) {
          $requirements["auto_updates_status_$severity"] = $requirement;
        }
      }
    }
    return $requirements;
  ...
  private function createRunLink(): ?TranslatableMarkup {
    ...
    $status_check_url = Url::fromRoute('auto_updates.status_check');
