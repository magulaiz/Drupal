...
class CronUpdateRunner implements CronInterface, LoggerAwareInterface {
  ...
  protected function runTerminalUpdateCommand(): void {
    ...
    try {
      ...
      if (method_exists(Error::class, 'logException')) {
        ...
        watchdog_exception('auto_updates', $throwable, 'Unable to start background update.');
      }
    }
  ...
  public function run() {
    ...
    $method = $this->configFactory->get('auto_updates.settings')
  ...
  final public function getMode(): string {
    $mode = $this->configFactory->get('auto_updates.settings')->get('unattended.level');
