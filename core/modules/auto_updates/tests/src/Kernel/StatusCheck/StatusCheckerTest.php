...
use Drupal\auto_updates\Validator\StagedProjectsValidator;
use Drupal\auto_updates_test\EventSubscriber\TestSubscriber1;
use Drupal\auto_updates_test_status_checker\EventSubscriber\TestSubscriber2;
use Drupal\package_manager\Event\StatusCheckEvent;
use Drupal\system\SystemManager;
use Drupal\Tests\auto_updates\Kernel\AutoUpdatesKernelTestBase;

/**
 * @coversDefaultClass \Drupal\auto_updates\Validation\StatusChecker
 * @group auto_updates
 * @internal
 */
class StatusCheckerTest extends AutoUpdatesKernelTestBase {
  ...
  protected function setUp(): void {
    ...
    $this->config('auto_updates.settings')->delete();
  }
  ...
  public function testGetResults(): void {
    $this->container->get('module_installer')
      ->install(['auto_updates', 'auto_updates_test_status_checker']);
  ...
  public function testRunOnInstall(): void {
    ...
    $this->container->get('module_installer')->install(['auto_updates_test_status_checker']);
  ...
  public function testRunOnUninstall(): void {
    ...
    // 'auto_updates' is installed.
    $this->container->get('module_installer')->install(['auto_updates', 'auto_updates_test_status_checker', 'help']);
    $expected_results_all = array_merge($checker_1_results, $checker_2_results);
    $this->assertCheckerResultsFromManager($expected_results_all);
    ...
    TestSubscriber1::setTestResult($checker_1_results, StatusCheckEvent::class);
    TestSubscriber2::setTestResult($checker_2_results, StatusCheckEvent::class);
    $this->container->get('module_installer')->uninstall(['auto_updates_test_status_checker']);
    $this->assertCheckerResultsFromManager($checker_1_results);
  ...
  public function testRunIfNeeded(): void {
    ...
    $this->container->get('module_installer')->install(['auto_updates', 'auto_updates_test_status_checker']);
