...
use Drupal\Tests\auto_updates\Traits\EmailNotificationsTestTrait;
...
/**
 * @covers \Drupal\auto_updates\ConsoleUpdateStage
 * @group auto_updates
 * @internal
 */
class ConsoleUpdateStageTest extends AutoUpdatesKernelTestBase {
...
END;
  ...
  public function testStageDestroyedOnError(string $event_class, string $exception_class): void {
    ...
    /** @var \Drupal\auto_updates\ConsoleUpdateStage $stage */
...
END;
  ...
  public function testMaintenanceModeIsOnDuringApply(): void {
    ...
    $this->runConsoleUpdateStage();
    $state->resetCache();
    // @see \Drupal\Tests\auto_updates\Kernel\TestCronUpdateRunner::apply()
    $this->assertTrue($this->logger->hasRecord('Unattended update was applied in maintenance mode.', RfcLogLevel::INFO));
    // @see \Drupal\Tests\auto_updates\Kernel\TestCronUpdateRunner::postApply()
