...
class CoreUpdateTest extends UpdateTestBase {
  ...
  private function assertUpdateSuccessful(string $expected_version): void {
    ...
    if ($type_select_field->find('named', ['option', 'auto_updates'])) {
      $assert_session->pageTextNotContains('No log messages available.');
      $page->selectFieldOption('Type', 'auto_updates');
    ...
    // \Drupal\auto_updates\Routing\RouteSubscriber::alterRoutes() sets
    // `_auto_updates_status_messages: skip` on the route for the path
    ...
    // @see \Drupal\auto_updates\Validation\AdminStatusCheckMessages::displayAdminPageMessages()
