...
class UserTokenReplaceTest extends BrowserTestBase {
  ...
  public function testUserTokenReplacement(): void {
    ...
    $link = Url::fromRoute(
      'user.page',
      [],
      ['language' => \Drupal::languageManager()->getLanguage($account->getPreferredLangcode()), 'absolute' => TRUE])->toString();
    foreach ($tests as $input => $expected) {
      $output = $token_service->replace(
        $input,
        ['user' => $account],
    ...
    $link = Url::fromRoute(
      'user.page',
      [],
      ['language' => \Drupal::languageManager()->getLanguage('de'), 'absolute' => TRUE])->toString();
    foreach ($tests as $input => $expected) {
      foreach ([$user1, $user2] as $account) {
        $output = $token_service->replace(
          $input, ['user' => $account], ['langcode' => 'de', 'callback' => 'user_mail_tokens', 'clear' => TRUE]);
