...
class UserValidationTest extends KernelTestBase {
  ...
  public function testValidation() {
    ...
    $this->assertEquals('The username '.$name.' is too long: it must be 60 characters or less.', $violations[0]->getMessage());
    ...
    $this->assertEquals('mail.0.value', $violations[0]->getPropertyPath());
    $this->assertEquals(''.$user->get('mail')->getFieldDefinition()->getLabel().': the email address can not be longer than Email::EMAIL_MAX_LENGTH characters.', $violations[0]->getMessage());
  ...
  protected function assertLengthViolation(EntityInterface $entity, string $field_name, int $length, int $count = 1, int $expected_index = 0): void {
    ...
    $this->assertEquals(''.$field_label.': may not be longer than '.$length.'.', $violations[$expected_index]->getMessage());
  }
