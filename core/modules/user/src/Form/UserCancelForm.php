...
class UserCancelForm extends ContentEntityConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!$form_state->isValueEmpty('access') && $form_state->isValueEmpty('user_cancel_confirm') && $this->entity->id() != $this->currentUser()->id()) {
      ...
      // Store cancelling method and whether to notify the user for
      // \Drupal\user\Controller\UserController::confirmCancel().
      \Drupal::service('user.data')->set('user', $this->entity->id(), 'cancel_method', $form_state->getValue('user_cancel_method'));
      \Drupal::service('user.data')->set('user', $this->entity->id(), 'cancel_notify', $form_state->getValue('user_cancel_notify'));
      $this->entity->save();
