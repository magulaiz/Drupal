...
class ContactStorageTestHooks
{
    ...
    public function entityBaseFieldInfo(\Drupal\Core\Entity\EntityTypeInterface $entity_type)
    {
        if ($entity_type->id() == 'contact_message') {
            $fields = [];
            $fields['id'] = \Drupal\Core\Field\BaseFieldDefinition::create('integer')->setLabel(\t('Message ID'))->setDescription(\t('The message ID.'))->setReadOnly(\TRUE)->setSetting('unsigned', \TRUE);
            return $fields;
        }
    }
    /**
    ...
    public function formContactFormFormAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state)
    {
        /** @var \Drupal\contact\ContactFormInterface $contact_form */
        $contact_form = $form_state->getFormObject()->getEntity();
        $form['send_a_pony'] = ['#type' => 'checkbox', '#title' => \t('Send submitters a voucher for a free pony.'), '#description' => \t('Enable to send an additional email with a free pony voucher to anyone who submits the form.'), '#default_value' => $contact_form->getThirdPartySetting('contact_storage_test', 'send_a_pony', \FALSE)];
