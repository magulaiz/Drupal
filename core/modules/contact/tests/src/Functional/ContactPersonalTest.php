...
class ContactPersonalTest extends BrowserTestBase {
  ...
  public function testPersonalContactAccess() {
    ...
    $this->drupalLogout();
    Role::load(RoleInterface::ANONYMOUS_ID)->grantPermission('access user contact forms')->save();
    $this->drupalGet('user/' . $this->contactUser->id() . '/contact');
    $this->assertSession()->statusCodeEquals(200);
    ...
    Role::load(RoleInterface::ANONYMOUS_ID)->revokePermission('access user contact forms')->save();
    $this->drupalGet('user/' . $this->contactUser->id() . '/contact');
    $this->assertSession()->statusCodeEquals(403);
    $this->assertCacheContext('user');
    $this->drupalGet('user/' . $this->adminUser->id() . '/contact');
