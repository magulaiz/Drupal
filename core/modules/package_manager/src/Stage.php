...
class Stage implements LoggerAwareInterface {
  ...
  public function __construct(ConfigFactoryInterface $config_factory, PathLocator $path_locator, BeginnerInterface $beginner, StagerInterface $stager, CommitterInterface $committer, FileSystemInterface $file_system, EventDispatcherInterface $event_dispatcher, SharedTempStoreFactory $temp_store_factory, TimeInterface $time, PathFactoryInterface $path_factory = NULL, FailureMarker $failure_marker = NULL) {
    ...
    if (!$config_factory instanceof UnusedConfigFactory) {
      ...
      @trigger_error('Calling ' . __METHOD__ . '() with the $config_factory argument is deprecated in auto_updates:8.x-2.6 and will be removed in auto_updates:3.0.0. See https://www.drupal.org/node/3325718.', E_USER_DEPRECATED);
    }
    ...
    if (self::TEMPSTORE_METADATA_KEY !== static::TEMPSTORE_METADATA_KEY) {
      @trigger_error('Overriding ' . __CLASS__ . '::TEMPSTORE_METADATA_KEY is deprecated in auto_updates:8.x-2.5 and will not be possible in auto_updates:3.0.0. There is no replacement. See https://www.drupal.org/node/3317450.', E_USER_DEPRECATED);
    ...
    if (self::TEMPSTORE_LOCK_KEY !== static::TEMPSTORE_LOCK_KEY) {
      @trigger_error('Overriding ' . __CLASS__ . '::TEMPSTORE_LOCK_KEY is deprecated in auto_updates:8.x-2.5 and will not be possible in auto_updates:3.0.0. There is no replacement. See https://www.drupal.org/node/3317450.', E_USER_DEPRECATED);
      \Drupal::logger('package_manager')
