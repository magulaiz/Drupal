...
class BlockDemoTest extends BrowserTestBase {
  ...
  public function testBlockDemo() {
    ...
    $this->drupalGet('admin/appearance/block/demo/' . $default_theme);
    $this->assertSession()->statusCodeEquals(200);
    $this->assertSession()->linkByHrefExists('admin/appearance/block');
    $this->assertSession()->linkByHrefNotExists('admin/appearance/block/list/' . $default_theme);
    ...
    foreach ($themes as $theme) {
      ...
      $this->drupalGet('admin/appearance/block/demo/' . $theme);
      $this->assertSession()->statusCodeEquals(200);
      // Confirm existence of link for "Exit block region demonstration".
      $this->assertSession()->linkByHrefExists('admin/appearance/block/list/' . $theme);
    }

    // Confirm access to the block demo page is denied for an invalid theme.
    $this->drupalGet('admin/appearance/block/demo/invalid_theme');
