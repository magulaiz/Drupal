...
class BlockTestHooks
{
    ...
    public function blockViewTestCacheAlter(array &$build, \Drupal\Core\Block\BlockPluginInterface $block)
    {
    ...
    public function blockBuildTestCacheAlter(array &$build, \Drupal\Core\Block\BlockPluginInterface $block)
    {
        ...
        if (\Drupal::state()->get('block_test_block_alter_cache_tag') !== \NULL) {
            $build['#cache']['tags'] = \Drupal\Core\Cache\Cache::mergeTags($build['#cache']['tags'], [\Drupal::state()->get('block_test_block_alter_cache_tag')]);
        }
