...
class NodeTestHooks
{
    ...
    public function nodeView(array &$build, \Drupal\node\NodeInterface $node, \Drupal\Core\Entity\Display\EntityViewDisplayInterface $display, $view_mode)
    {
        ...
        if ($view_mode == 'rss') {
            ...
            $node->rss_elements[] = ['key' => 'testElement', 'value' => \t('Value of testElement RSS element for node @nid.', ['@nid' => $node->id()])];
            // Add content that should be displayed only in the RSS feed.
            $build['extra_feed_content'] = ['#markup' => '<p>' . \t('Extra data that should appear only in the RSS feed for node @nid.', ['@nid' => $node->id()]) . '</p>', '#weight' => 10];
        }
        if ($view_mode != 'rss') {
            // Add content that should NOT be displayed in the RSS feed.
            $build['extra_non_feed_content'] = ['#markup' => '<p>' . \t('Extra data that should appear everywhere except the RSS feed for node @nid.', ['@nid' => $node->id()]) . '</p>'];
        }
    }
    /**
    ...
    public function nodeBuildDefaultsAlter(array &$build, \Drupal\node\NodeInterface &$node, $view_mode = 'full')
    {
        if ($view_mode == 'rss') {
     ...
     */
    #[Hook('node_grants')]
    public function nodeGrants(\Drupal\Core\Session\AccountInterface $account, $operation)
    {
    ...
    public function nodeAccessRecords(\Drupal\node\NodeInterface $node)
    {
        ...
        if ($node->getType() == 'article') {
            ...
            $grants[] = ['realm' => 'test_article_realm', 'gid' => 1, 'grant_view' => 1, 'grant_update' => 0, 'grant_delete' => 0];
        } elseif ($node->getType() == 'page') {
            // Create grant in arbitrary page_realm for page nodes.
            $grants[] = ['realm' => 'test_page_realm', 'gid' => 1, 'grant_view' => 1, 'grant_update' => 0, 'grant_delete' => 0];
        }
        return $grants;
    }
    /**
    ...
    public function nodeAccessRecordsAlter(&$grants, \Drupal\node\NodeInterface $node)
    {
        if (!empty($grants)) {
    ...
    public function nodeGrantsAlter(&$grants, \Drupal\Core\Session\AccountInterface $account, $operation)
    {
     ...
     */
    #[Hook('node_presave')]
    public function nodePresave(\Drupal\node\NodeInterface $node)
    {
     ...
     */
    #[Hook('node_update')]
    public function nodeUpdate(\Drupal\node\NodeInterface $node)
    {
    ...
    public function entityViewModeAlter(&$view_mode, \Drupal\Core\Entity\EntityInterface $entity)
    {
     ...
     */
    #[Hook('node_insert')]
    public function nodeInsert(\Drupal\node\NodeInterface $node)
    {
