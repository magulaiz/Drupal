...
class ConfigImportUITest extends BrowserTestBase {
  ...
  public function testImport(): void {
    ...
    // Enable the Automated Cron module during import.
    ...
    $expected = ['automated_cron', 'text', 'options'];
    $installed = \Drupal::state()->get('config_import_test_modules_installed.list');
    $this->assertSame($expected, $installed, 'Automated Cron, Text and Options modules installed in the correct order.');
    ...
    $uninstalled = \Drupal::state()->get('ConfigImportUITest.core.extension.modules_uninstalled', []);
    $expected = ['options', 'text', 'automated_cron'];
    $this->assertSame($expected, $uninstalled, 'Options, Text and Automated Cron modules uninstalled in the correct order.');
