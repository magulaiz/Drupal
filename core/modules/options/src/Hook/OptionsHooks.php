...
class OptionsHooks {
  ...
  public function fieldStorageConfigUpdateForbid(FieldStorageConfigInterface $field_storage, FieldStorageConfigInterface $prior_field_storage): void {
    if ($field_storage->getTypeProvider() == 'options' && $field_storage->hasData()) {
      ...
      $allowed_values_func = $field_storage->getSetting('allowed_values_function');
      $prior_allowed_values = $prior_field_storage->getSetting('allowed_values');
      $prior_allowed_values_func = $prior_field_storage->getSetting('allowed_values_function');

      // If values from current or previous config are dynamically generated, then
      // don't attempt to compare values. Assume dev knows what they are doing
      // and allow the change.
      if (!empty($allowed_values_func) || !empty($prior_allowed_values_func)) {
        return;
      }
