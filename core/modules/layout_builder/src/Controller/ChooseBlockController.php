...
class ChooseBlockController implements ContainerInjectionInterface {
  ...
  public function build(SectionStorageInterface $section_storage, int $delta, $region) {
    if ($section_storage->getPluginDefinition()->get('allow_inline_blocks') && $this->entityTypeManager->hasDefinition('block_content_type') && $types = $this->entityTypeManager->getStorage('block_content_type')->loadMultiple()) {
      if (count($types) === 1) {
        $type = reset($types);
