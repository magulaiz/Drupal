...
class LayoutBuilderTestHooks
{
    ...
    {
        $extra['node']['bundle_with_section_field']['display']['layout_builder_test'] = ['label' => \t('Extra label'), 'description' => \t('Extra description'), 'weight' => 0];
        $extra['node']['bundle_with_section_field']['display']['layout_builder_test_2'] = ['label' => \t('Extra Field 2'), 'description' => \t('Extra Field 2 description'), 'weight' => 0, 'visible' => \FALSE];
        return $extra;
    }
    /**
    ...
    public function formLayoutBuilderConfigureBlockAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state, $form_id)
    {
        /** @var \Drupal\layout_builder\Form\ConfigureBlockFormBase $form_object */
        $form_object = $form_state->getFormObject();
        $form['layout_builder_test']['storage'] = ['#type' => 'item', '#title' => 'Layout Builder Storage: ' . $form_object->getSectionStorage()->getStorageId()];
        $form['layout_builder_test']['section'] = ['#type' => 'item', '#title' => 'Layout Builder Section: ' . $form_object->getCurrentSection()->getLayoutId()];
        $form['layout_builder_test']['component'] = ['#type' => 'item', '#title' => 'Layout Builder Component: ' . $form_object->getCurrentComponent()->getPluginId()];
    }
    /**
    ...
    public function formLayoutBuilderConfigureSectionAlter(&$form, \Drupal\Core\Form\FormStateInterface $form_state, $form_id)
    {
        ...
        $form['layout_builder_test']['storage'] = ['#type' => 'item', '#title' => 'Layout Builder Storage: ' . $form_object->getSectionStorage()->getStorageId()];
        $form['layout_builder_test']['section'] = ['#type' => 'item', '#title' => 'Layout Builder Section: ' . $form_object->getCurrentSection()->getLayoutId()];
        $form['layout_builder_test']['layout'] = ['#type' => 'item', '#title' => 'Layout Builder Layout: ' . $form_object->getCurrentLayout()->getPluginId()];
    }
    /**
     * Implements hook_entity_form_display_alter().
     */
    #[Hook('entity_form_display_alter', module: 'layout_builder')]
    public function layoutBuilderEntityFormDisplayAlter(\Drupal\Core\Entity\Display\EntityFormDisplayInterface $form_display, array $context)
    {
    ...
    public function systemBreadcrumbAlter(\Drupal\Core\Breadcrumb\Breadcrumb &$breadcrumb, \Drupal\Core\Routing\RouteMatchInterface $route_match, array $context)
    {
        $breadcrumb->addLink(\Drupal\Core\Link::fromTextAndUrl('External link', \Drupal\Core\Url::fromUri('http://www.example.com')));
    }
    /**
    ...
    public function moduleImplementsAlter(&$implementations, $hook)
    {
        if ($hook === 'system_breadcrumb_alter') {
            ...
            $implementations = ['layout_builder_test' => $group] + $implementations;
        }
    }
    /**
