...
abstract class InlineBlockTestBase extends WebDriverTestBase {
   ...
   */
  protected function addInlineBlockToLayout(string $title, string $body) {
    $assert_session = $this->assertSession();
    $page = $this->getSession()->getPage();
   ...
   */
  protected function configureInlineBlock(string $old_body, string $new_body, string $block_css_locator = NULL) {
   ...
   */
  protected function assertDialogClosedAndTextVisible(string $text, $css_locator = NULL) {
    $assert_session = $this->assertSession();
    $assert_session->assertNoElementAfterWait('css', '#drupal-off-canvas');
   ...
   */
  protected function createBlockContentType(string $id, string $label) {
