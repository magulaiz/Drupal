...
class MultivalueFieldTest extends WebDriverTestBase {
  ...
  protected function setUp(): void {
    ...
    // Create a second node with no attached field items.
    $this->createNode([
      'type' => 'bundle_with_section_field',
      'title' => 'Mesopotamia',
    ]);
  ...
  public function testMultiValueField($params) {
    ...
    $this->updateMultiValueFieldSettings($assert_session, $page, $params);
    ...
    // Test saving layout on a node on which no field items have been set.
    $page = $this->getSession()->getPage();
  ...
  /**
   * Configure settings for Multiple Value fields in a Layout.
   */
  protected function updateMultiValueFieldSettings($assert_session, $page, $params) {
    ...
    $this->assertNotEmpty($assert_session->waitForElementVisible('css', '[name="settings[multivalue_wrapper][items_to_display]"]'));
