...
abstract class MigrateTestBase extends KernelTestBase implements MigrateMessageInterface {
   ...
   *   IDs.
   */
  protected function prepareMigrations(array $id_mappings): void {
   ...
   */
  protected function executeMigrations(array $ids): void {
    $manager = $this->container->get('plugin.manager.migration');
   ...
   */
  protected function mockFailure($migration, array $row, $status = MigrateIdMapInterface::STATUS_FAILED): void {
    if (is_string($migration)) {
   ...
   */
  protected function setTestLogger(): void {
