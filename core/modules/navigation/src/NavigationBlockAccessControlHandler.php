...
/**
 * Defines the access control handler for the navigation block entity type.
 *
 * @see \Drupal\navigation\Entity\NavigationBlock
 */
class NavigationBlockAccessControlHandler extends EntityAccessControlHandler implements EntityHandlerInterface {
  ...
  /**
   * Constructs the navigation_block access control handler instance.
   *
  ...
  protected function checkAccess(EntityInterface $entity, $operation, AccountInterface $account) {
    ...
    // Don't grant access to disabled navigation blocks.
    if (!$entity->status()) {
      ...
      if ($missing_context) {
        ...
        // metadata, and don't know based on what conditions the navigation
        // block is accessible or not. Make sure the result cannot be cached.
      ...
      elseif ($this->resolveConditions($conditions, 'and') !== FALSE) {
        ...
        $navigation_block_plugin = $entity->getPlugin();
        try {
          if ($navigation_block_plugin instanceof ContextAwarePluginInterface) {
            $contexts = $this->contextRepository->getRuntimeContexts(array_values($navigation_block_plugin->getContextMapping()));
            $this->contextHandler->applyContextMapping($navigation_block_plugin, $contexts);
          }
          $access = $navigation_block_plugin->access($account, TRUE);
        }
        catch (MissingValueContextException $e) {
          // The contexts exist but have no value. Deny access without
          // disabling caching.
          ...
          // If any context is missing then we might be missing cacheable
          // metadata, and don't know based on what conditions the navigation
          // block is accessible or not. Make sure the result cannot be
          // cached.
          $access = AccessResult::forbidden()->setCacheMaxAge(0);
        }
      }
        ...
        $reason = count($conditions) > 1
          ? "One of the navigation block visibility conditions ('%s') denied access."
          : "The navigation block visibility condition '%s' denied access.";
      ...
      // Ensure that access is evaluated again when the navigation block
      // changes.
