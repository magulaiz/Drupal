  ...
  protected function addSubQueryJoin($tids): void {
    ...
    if ($this->options['depth'] !== 0) {
      ...
      foreach (range(1, abs($this->options['depth'])) as $count) {
        ...
        $left_join = "[tn].[tid]";
        if ($this->options['depth'] > 0) {
          $union_query->join('taxonomy_term__parent', "th", $connection->condition('AND')->compare($left_join, 'th.entity_id'));
          $left_join = "[th].[$left_field]";
        }
        foreach (range(1, $count) as $inner_count) {
          $union_query->join('taxonomy_term__parent', "th$inner_count", $connection->condition('AND')->compare($left_join, "th$inner_count.$right_field"));
