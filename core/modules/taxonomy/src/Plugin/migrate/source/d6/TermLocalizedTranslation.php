...
class TermLocalizedTranslation extends Term {
  ...
  public function query() {
    ...
    $query->leftJoin('i18n_strings', 'i18n', $this->getDatabase()->condition('AND')->where('CAST([td].[tid] AS CHAR(255)) = [i18n].[objectid]'));
    $query->condition('i18n.type', 'term');
    $query->addField('i18n', 'lid');
    $query->addField('i18n', 'property');

    // Add in the translation for the property.
    $query->innerJoin('locales_target', 'lt', $this->getDatabase()->condition('AND')->compare('i18n.lid', 'lt.lid'));
  ...
  public function prepareRow(Row $row) {
    ...
    $query->leftJoin('locales_target', 'lt', $this->getDatabase()->condition('AND')->compare('i18n.lid', 'lt.lid'));
