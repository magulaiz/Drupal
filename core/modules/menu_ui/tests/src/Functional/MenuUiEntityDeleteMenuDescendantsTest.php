...
final class MenuUiEntityDeleteMenuDescendantsTest extends BrowserTestBase {
  ...
  public function testDeleteEntityWithMenuAndChildren(string $testEntityType, bool $hasMultipleMenuItemChildren): void {
    ...
    if ($hasMultipleMenuItemChildren) {
      $this->assertSession()->pageTextContains($testingAsMenuDelete ? 'The menu items for this custom menu link has 2 children menu items. Deleting this custom menu link will cause these children menu items to move to the same level as the deleted menu item. Affected menu items include:' : 'The menu items for this test entity has 2 children menu items. Deleting this test entity will cause these children menu items to move to the same level as the deleted menu item. Affected menu items include:');
      ...
      $this->assertSession()->pageTextContains($testingAsMenuDelete ? 'The menu item for this custom menu link has 1 child menu item. Deleting this custom menu link will cause this child menu item to move to the same level as the deleted menu item. The affected menu item is:' : 'The menu item for this test entity has 1 child menu item. Deleting this test entity will cause this child menu item to move to the same level as the deleted menu item. The affected menu item is:');
      $this->assertSession()->linkNotExists('Menu link for Test entity #1');
      $this->assertSession()->linkNotExists('Menu link for Test entity #2');
      $this->assertSession()->linkExists('Menu link for Test entity #3');
