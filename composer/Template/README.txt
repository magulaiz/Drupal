Composer-Ready Project Template
===============================

Thanks for using the Drupal project template.
...
The recommended project template serves as a starting point for creating a
Composer-managed Drupal site. Once you have installed a template project and
...
The recommended project creates a new Drupal site with a "relocated document
root". This means that the files "index.php" and the "core" directory and so on
are placed inside a subfolder named "web" rather than being placed next to
"composer.json" and the "vendor" directory at the project root. This layout is
recommended because it allows you to configure your web server to only provide
access to files inside the "web" directory. Keeping the vendor directory outside
of the web server's document root is better for security.
...
The template project uses drupal/core-composer-scaffold to place the scaffold
